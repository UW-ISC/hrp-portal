    ...
    <div class="row wdt-edit-dialog-fields-block">
        ...
        foreach ($this->getColumnsByHeaders() as $dataColumn_key => $dataColumn) {
            ...
            <div
                ...
                <!-- .control-label -->
                <label for="<?php echo esc_attr($this->getId()) ?>_<?php echo esc_attr($dataColumn_key) ?>"
                       class="col-sm-3 control-label">
                    <?php echo esc_html($dataColumn->getTitle()); ?>
                    :<?php if ($dataColumn->isNotNull()) { ?> * <?php } ?>
                </label>
                <!--/ .control-label -->
                    ...
                    <div class="fg-line">
                        ...
                        if ($dataColumn->getInputType() === 'textarea' ||
                            ...
                            <select id="<?php echo esc_attr($this->getId()) ?>_<?php echo esc_attr($dataColumn_key) ?>"
                                    ...
                                    title="<?php echo esc_attr(WDTTools::getTranslationStringsColumnFilter()['nothingSelected_columnfilter']) ?>"
...
$skinsWithNewEditButtons = ['aqua', 'purple', 'dark', 'raspberry-cream', 'mojito', 'dark-mojito']; ?>
...
<div id="<?php echo esc_attr($this->getId()) ?>_edit_dialog_buttons" class="wdt-edit-dialog-button-block"
    ...
    <button id="<?php echo $this->getId() ?>_prev_edit_dialog" class="btn bgm-gray btn-icon-text wdt-prev-edit-button">
        <i class="<?php if (in_array($currentSkin, $skinsWithNewEditButtons)) {
            echo 'wpdt-icon-chevron-left';
        } else {
            echo 'wpdt-icon-step-backward';
        } ?>"></i>
        <?php if (!in_array($currentSkin, ['mojito',
            'raspberry-cream',
            'dark-mojito'])) esc_html_e('Prev', 'wpdatatables'); ?>
    </button>
    <button id="<?php echo $this->getId() ?>_next_edit_dialog" class="btn bgm-gray btn-icon-text wdt-next-edit-button">
        <?php if (!in_array($currentSkin, ['mojito',
            'raspberry-cream',
            'dark-mojito'])) esc_html_e('Next', 'wpdatatables'); ?>
        <i class="<?php if (in_array($currentSkin, $skinsWithNewEditButtons)) {
            echo 'wpdt-icon-chevron-right';
        } else {
            echo 'wpdt-icon-step-forward';
        } ?>"></i>
    </button>
    <?php if (!empty(array_intersect(['all', 'new_entry'], $this->getEditButtonsDisplayed()))) { ?>
        <button id="<?php echo $this->getId() ?>_apply_edit_dialog"
                class="btn btn-success btn-icon-text wdt-apply-edit-button">
            <i class="<?php if (in_array($currentSkin, $skinsWithNewEditButtons)) {
                echo 'wpdt-icon-check-circle-full';
            } else {
                echo 'wpdt-icon-check';
            } ?>"></i>
    ...
    <?php if ($this->isEnableDuplicateButton() &&
        ...
        <button id="<?php echo $this->getId() ?>_apply_duplicate_dialog"
                class="btn btn-success btn-icon-text wdt-apply-duplicate-button hidden">
            <i class="<?php if (in_array($currentSkin, $skinsWithNewEditButtons)) {
                echo 'wpdt-icon-check-circle-full';
            } else {
                echo 'wpdt-icon-check';
    ...
    <button id="<?php echo $this->getId() ?>_ok_edit_dialog" class="btn btn-success btn-icon-text wdt-ok-edit-button">
        <i class="<?php if (in_array($currentSkin, $skinsWithNewEditButtons)) {
            echo 'wpdt-icon-check-circle';
        } else {
            echo 'wpdt-icon-check-double-reg';
        } ?>"></i>
        <?php if (in_array($currentSkin, $skinsWithNewEditButtons)) {
            esc_html_e('Submit', 'wpdatatables');
        } else {
            esc_html_e('OK', 'wpdatatables');
        } ?>
    </button>
</div>
