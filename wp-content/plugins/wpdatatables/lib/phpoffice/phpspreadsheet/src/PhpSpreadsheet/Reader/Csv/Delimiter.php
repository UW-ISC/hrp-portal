...
namespace WPDT\PhpOffice\PhpSpreadsheet\Reader\Csv;
...
class Delimiter
{
    ...
    public function infer() : ?string
    {
        ...
        foreach (self::POTENTIAL_DELIMETERS as $delimiter) {
            ...
            \sort($series);
            $median = $this->numberLines % 2 ? $series[$middleIdx] : ($series[$middleIdx] + $series[$middleIdx + 1]) / 2;
            if ($median === 0) {
                continue;
            }
            $meanSquareDeviations[$delimiter] = \array_reduce($series, function ($sum, $value) use($median) {
    ...
    {
        $line = '';
        $enclosure = ($this->escapeCharacter === '' ? '' : '(?<!' . \preg_quote($this->escapeCharacter, '/') . ')') . \preg_quote($this->enclosure, '/');
        do {
