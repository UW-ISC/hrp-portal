...
use WPDT\PhpOffice\PhpSpreadsheet\RichText\Run;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\StringHelper;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\Xls;
...
class Worksheet extends BIFFwriter
{
     ...
     */
    public function __construct(&$str_total, &$str_unique, &$str_table, &$colors, Parser $parser, $preCalculateFormulas, \WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $phpSheet)
    {
        ...
        $this->outlineBelow = \true;
        $this->outlineRight = \true;
        $this->outlineOn = \true;
        $this->fontHashIndex = [];
    ...
    public function close() : void
    {
        ...
        if (($defaultWidth = $phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
            $defaultWidth = \WPDT\PhpOffice\PhpSpreadsheet\Shared\Font::getDefaultColumnWidthByFont($phpSheet->getParentOrThrow()->getDefaultStyle()->getFont());
        }
            ...
            foreach ($arrConditionalStyles as $cellCoordinate => $conditionalStyles) {
                ...
                foreach ($conditionalStyles as $conditional) {
                    ...
                    if ($conditional->getConditionType() === Conditional::CONDITION_EXPRESSION || $conditional->getConditionType() === Conditional::CONDITION_CELLIS) {
                        ...
                        if ($cfHeaderWritten === \true && !isset($arrConditional[$conditional->getHashCode()])) {
    ...
    private function writeBIFF8CellRangeAddressFixed($range)
    {
        ...
        $firstCellCoordinates = Coordinate::indexesFromString($firstCell);
        // e.g. [0, 1]
        $lastCellCoordinates = Coordinate::indexesFromString($lastCell);
        // e.g. [1, 6]
        ...
        if (isset($calculatedValue)) {
            ...
            if (\is_bool($calculatedValue)) {
                ...
                if (isset($errorCodes[$calculatedValue])) {
                    // Error value
                    $num = \pack('CCCvCv', 0x2, 0x0, ErrorCode::error($calculatedValue), 0x0, 0x0, 0xffff);
    ...
    private function writeUrlExternal($row1, $col1, $row2, $col2, $url) : void
    {
        ...
        $dir_short = (string) \preg_replace('/\\.\\.\\\\/', '', $dir_long) . "\x00";
    ...
    private function writeSetup() : void
    {
        ...
        // Paper size
        $iScale = $this->phpSheet->getPageSetup()->getScale() ?: 100;
        ...
        $fLeftToRight = $this->phpSheet->getPageSetup()->getPageOrder() === PageSetup::PAGEORDER_DOWN_THEN_OVER ? 0x0 : 0x1;
        // Page orientation
    ...
    private function writeProtect() : void
    {
        ...
        if ($this->phpSheet->getProtection()->getSheet() !== \true) {
            return;
        }
        $record = 0x12;
        // Record identifier
    ...
    private function writeScenProtect() : void
    {
        ...
        if ($this->phpSheet->getProtection()->getScenarios() !== \true) {
            return;
        }
        $record = 0xdd;
        // Record identifier
    ...
    private function writeObjectProtect() : void
    {
        ...
        if ($this->phpSheet->getProtection()->getObjects() !== \true) {
            return;
        }
        $record = 0x63;
        // Record identifier
    ...
    private function writePassword() : void
    {
        ...
        if ($this->phpSheet->getProtection()->getSheet() !== \true || !$this->phpSheet->getProtection()->getPassword() || $this->phpSheet->getProtection()->getAlgorithm() !== '') {
            return;
        }
        $record = 0x13;
        // Record identifier
     ...
     * Get Escher object.
     */
    public function getEscher() : ?\WPDT\PhpOffice\PhpSpreadsheet\Shared\Escher
    {
        return $this->escher;
    }
    /**
     * Set Escher object.
     */
    public function setEscher(?\WPDT\PhpOffice\PhpSpreadsheet\Shared\Escher $escher) : void
    {
        $this->escher = $escher;
    }
    /**
     ...
     * Write CFRule Record.
     */
    private function writeCFRule(ConditionalHelper $conditionalFormulaHelper, Conditional $conditional, string $cellRange) : void
    {
        $record = 0x1b1;
        // Record identifier
        $type = null;
        ...
        $flags |= 1 == self::$always1 ? 0x4000 : 0;
        // Top left to Bottom right border
        $flags |= 1 == self::$always1 ? 0x8000 : 0;
        ...
        if ($bFormatFont == 1) {
            ...
            switch ($conditional->getStyle()->getFont()->getUnderline()) {
                case \WPDT\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_NONE:
                    $dataBlockFont .= \pack('C', 0x0);
                    $fontUnderline = 0;
                    break;
                case \WPDT\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_DOUBLE:
                    $dataBlockFont .= \pack('C', 0x2);
                    $fontUnderline = 0;
                    break;
                case \WPDT\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_DOUBLEACCOUNTING:
                    $dataBlockFont .= \pack('C', 0x22);
                    $fontUnderline = 0;
                    break;
                case \WPDT\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE:
                    $dataBlockFont .= \pack('C', 0x1);
                    $fontUnderline = 0;
                    break;
                case \WPDT\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLEACCOUNTING:
            ...
            // Font color index
            $colorIdx = Style\ColorMap::lookup($conditional->getStyle()->getFont()->getColor(), 0x0);
            ...
            $optionsFlags |= 1 == self::$always1 ? 0x8 : 0;
            $optionsFlags |= 1 == self::$always1 ? 0x10 : 0;
            $optionsFlags |= 1 == self::$always0 ? 0x20 : 0;
            $optionsFlags |= 1 == self::$always1 ? 0x80 : 0;
