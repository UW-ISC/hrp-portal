...
use WPDT\PhpOffice\PhpSpreadsheet\IOFactory;
...
use WPDT\PhpOffice\PhpSpreadsheet\Writer\IWriter;
...
class Sample
{
    ...
    public function renderChart(Chart $chart, string $fileName) : void
    {
        ...
        Settings::setChartRenderer(\WPDT\PhpOffice\PhpSpreadsheet\Chart\Renderer\MtJpGraphRenderer::class);
    ...
    public function titles(string $category, string $functionName, ?string $description = null) : void
    {
        ...
        $description === null ? $this->log(\sprintf('Function: %s()', \rtrim($functionName, '()'))) : $this->log(\sprintf('Function: %s() - %s.', \rtrim($functionName, '()'), \rtrim($description, '.')));
    }
    ...
    public function logEndingNotes() : void
    {
        ...
        $this->log('Peak memory usage: ' . \memory_get_peak_usage(\true) / 1024 / 1024 . 'MB');
    }
    /**
    ...
    {
        $callEndTime = \microtime(\true);
        $callTime = $callEndTime - $callStartTime;
        $reflection = new ReflectionClass($writer);
        $format = $reflection->getShortName();
        $message = $this->isCli() === \true ? "Write {$format} format to {$path}  in " . \sprintf('%.4f', $callTime) . ' seconds' : "Write {$format} format to <code>{$path}</code>  in " . \sprintf('%.4f', $callTime) . ' seconds';
        $this->log($message);
    }
    /**
