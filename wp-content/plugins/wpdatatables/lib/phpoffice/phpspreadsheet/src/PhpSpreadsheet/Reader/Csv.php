...
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Csv\Delimiter;
...
class Csv extends BaseReader
{
    ...
    public function loadSpreadsheetFromString(string $contents) : Spreadsheet
    {
        ...
        $spreadsheet = new Spreadsheet();
        // Load into this instance
        return $this->loadStringOrFile('data://text/plain,' . \urlencode($contents), $spreadsheet, \true);
    }
        ...
        while (\is_array($rowData)) {
            ...
            foreach ($rowData as $rowDatum) {
                ...
                if (($rowDatum !== '' || $this->preserveNullString) && $this->readFilter->readCell($columnLetter, $currentRow)) {
                    ...
                    $sheet->getCell($columnLetter . $outRow)->getStyle()->getNumberFormat()->setFormatCode($numberFormatMask);
                    // Set cell value
    ...
    private function convertFormattedNumber(&$rowDatum) : string
    {
        ...
        if ($this->castFormattedNumberToNumeric === \true && \is_string($rowDatum)) {
            $numeric = \str_replace([StringHelper::getThousandsSeparator(), StringHelper::getDecimalSeparator()], ['', '.'], $rowDatum);
            if (\is_numeric($numeric)) {
                $decimalPos = \strpos($rowDatum, StringHelper::getDecimalSeparator());
                if ($this->preserveNumericFormatting === \true) {
                    $numberFormatMask = \strpos($rowDatum, StringHelper::getThousandsSeparator()) !== \false ? '#,##0' : '0';
                    if ($decimalPos !== \false) {
    ...
    public function canRead(string $filename) : bool
    {
        ...
        $supportedTypes = ['application/csv', 'text/csv', 'text/plain', 'inode/x-empty'];
        return \in_array($type, $supportedTypes, \true);
    }
     ...
     */
    private static function getCsv($stream, ?int $length = null, string $separator = ',', string $enclosure = '"', ?string $escape = null)
    {
    ...
    public static function affectedByPhp9(string $filename, string $inputEncoding = 'UTF-8', ?string $delimiter = null, string $enclosure = '"', string $escapeCharacter = '\\') : bool
    {
        ...
        $reader1 = new self();
        $reader1->setInputEncoding($inputEncoding)->setTestAutoDetect(\true)->setEscapeCharacter($escapeCharacter)->setDelimiter($delimiter)->setEnclosure($enclosure);
        $spreadsheet1 = $reader1->load($filename);
        $sheet1 = $spreadsheet1->getActiveSheet();
        $array1 = $sheet1->toArray(null, \false, \false);
        $spreadsheet1->disconnectWorksheets();
        $reader2 = new self();
        $reader2->setInputEncoding($inputEncoding)->setTestAutoDetect(\false)->setEscapeCharacter('')->setDelimiter($delimiter)->setEnclosure($enclosure);
