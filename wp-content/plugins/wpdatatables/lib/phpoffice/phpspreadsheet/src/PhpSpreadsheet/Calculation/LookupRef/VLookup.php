...
class VLookup extends LookupBase
{
    ...
    private static function vLookupSearch($lookupValue, array $lookupArray, $column, bool $notExactMatch) : ?int
    {
        ...
        foreach ($lookupArray as $rowKey => $rowData) {
            ...
            if ($notExactMatch && ($bothNumeric && $rowData[$column] > $lookupValue || $bothNotNumeric && $cellDataLower > $lookupLower)) {
                break;
            }
            $rowNumber = self::checkMatch($bothNumeric, $bothNotNumeric, $notExactMatch, $rowKey, $cellDataLower, $lookupLower, $rowNumber);
        }
        return $rowNumber;
    }
}
