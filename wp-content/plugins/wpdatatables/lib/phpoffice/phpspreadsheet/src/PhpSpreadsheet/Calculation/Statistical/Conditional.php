...
use WPDT\PhpOffice\PhpSpreadsheet\Calculation\Database\DAverage;
use WPDT\PhpOffice\PhpSpreadsheet\Calculation\Database\DCount;
use WPDT\PhpOffice\PhpSpreadsheet\Calculation\Database\DMax;
use WPDT\PhpOffice\PhpSpreadsheet\Calculation\Database\DMin;
use WPDT\PhpOffice\PhpSpreadsheet\Calculation\Database\DSum;
...
class Conditional
{
    ...
    public static function COUNTIF($range, $condition)
    {
        ...
        $range = \array_filter(Functions::flattenArray($range), function ($value) {
    ...
    {
        $database = [];
        $database[] = \array_merge([self::VALUE_COLUMN_NAME], Functions::flattenArray($args[0]));
    ...
    private static function buildDataSet(int $startOffset, array $database, ...$args) : array
    {
        ...
        $argumentCount = \count($args);
        for ($argument = $startOffset; $argument < $argumentCount; $argument += 2) {
            $database[] = \array_merge([\sprintf(self::CONDITIONAL_COLUMN_NAME, $pairCount)], Functions::flattenArray($args[$argument]));
            ++$pairCount;
        }
        return \array_map(
            /** @scrutinizer ignore-type */
