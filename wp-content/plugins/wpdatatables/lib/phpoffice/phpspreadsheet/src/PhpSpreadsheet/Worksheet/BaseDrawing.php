...
class BaseDrawing implements IComparable
{
     ...
     * @var bool
     */
    protected $resizeProportional = \true;
    /**
     * Rotation.
     *
     * @var int
            ...
            if ($overrideOld) {
                ...
                while ($iterator->valid()) {
                    if ($iterator->current()->getHashCode() === $this->getHashCode()) {
                        $this->worksheet->getDrawingCollection()->offsetUnset(
    ...
    {
        return \md5($this->name . $this->description . ($this->worksheet === null ? '' : (string) $this->worksheet->getHashInt()) . $this->coordinates . $this->offsetX . $this->offsetY . $this->coordinates2 . $this->offsetX2 . $this->offsetY2 . $this->width . $this->height . $this->rotation . $this->shadow->getHashCode() . __CLASS__);
    }
    /**
    ...
    {
        return \in_array($this->editAs, self::VALID_EDIT_AS, \true);
    }
}
