...
class AddressHelper
{
     ...
     */
    public static function convertToA1(string $address, int $currentRowNumber = 1, int $currentColumnNumber = 1, bool $useLocale = \true) : string
    {
        ...
        if ($rowReference[0] === '[') {
            $rowReference = $currentRowNumber + (int) \trim($rowReference, '[]');
        }
        ...
        if (\is_string($columnReference) && $columnReference[0] === '[') {
            $columnReference = $currentColumnNumber + (int) \trim($columnReference, '[]');
        }
