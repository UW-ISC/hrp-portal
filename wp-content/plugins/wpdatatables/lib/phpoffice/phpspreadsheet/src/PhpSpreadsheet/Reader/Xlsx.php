...
use WPDT\PhpOffice\PhpSpreadsheet\Cell\Hyperlink;
use WPDT\PhpOffice\PhpSpreadsheet\DefinedName;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Security\XmlScanner;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\AutoFilter;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Chart;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\ColumnAndRowAttributes;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\ConditionalStyles;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\DataValidations;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Hyperlinks;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Namespaces;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\PageSetup;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Properties as PropertyReader;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\SharedFormula;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\SheetViewOptions;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\SheetViews;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Styles;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\TableReader;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Theme;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xlsx\WorkbookView;
...
use WPDT\PhpOffice\PhpSpreadsheet\Shared\Drawing;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\File;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\Font;
...
use WPDT\PhpOffice\PhpSpreadsheet\Style\Font as StyleFont;
use WPDT\PhpOffice\PhpSpreadsheet\Style\NumberFormat;
use WPDT\PhpOffice\PhpSpreadsheet\Style\Style;
use WPDT\PhpOffice\PhpSpreadsheet\Worksheet\HeaderFooterDrawing;
...
class Xlsx extends BaseReader
{
    ...
    private function loadZip(string $filename, string $ns = '', bool $replaceUnclosedBr = \false) : SimpleXMLElement
    {
        ...
        $rels = @\simplexml_load_string($this->getSecurityScannerOrThrow()->scan($contents), SimpleXMLElement::class, $this->parseHuge ? \LIBXML_PARSEHUGE : 0, $ns);
    ...
    {
        $contents = $this->getFromZipArchive($this->zip, $filename);
        $rels = \simplexml_load_string($this->getSecurityScannerOrThrow()->scan($contents), SimpleXMLElement::class, $this->parseHuge ? \LIBXML_PARSEHUGE : 0, $ns === '' ? $ns : '');
        return self::testSimpleXml($rels);
    }
        ...
        foreach ($rels->Relationship as $relx) {
            ...
            if ($mainNS !== '') {
                ...
                if ($xmlWorkbook->sheets) {
                    ...
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        $tmpInfo = ['worksheetName' => (string) self::getAttributes($eleSheet)['name'], 'lastColumnLetter' => 'A', 'lastColumnIndex' => 0, 'totalRows' => 0, 'totalColumns' => 0];
                        ...
                        $xml = new XMLReader();
                        $xml->xml($this->getSecurityScannerOrThrow()->scan($this->getFromZipArchive($this->zip, $fileWorksheetPath)), null, $this->parseHuge ? \LIBXML_PARSEHUGE : 0);
     ...
     */
    private function castToFormula(?SimpleXMLElement $c, string $r, string &$cellDataType, &$value, &$calculatedValue, string $castBaseType, bool $updateSharedCells = \true) : void
    {
        if ($c === null) {
            return;
        }
    ...
    protected function loadSpreadsheetFromFile(string $filename) : Spreadsheet
    {
        ...
        $addingFirstCellStyleXf = \true;
        $addingFirstCellXf = \true;
        $unparsedLoadedData = [];
        $this->zip = $zip = new ZipArchive();
        $zip->open($filename);
        ...
        foreach ($rels->Relationship as $relx) {
            ...
            switch ($relType) {
                ...
                case "{$xmlNamespaceBase}/officeDocument":
                    ...
                    $relType = "rel:Relationship[@Type='" . "{$xmlNamespaceBase}/styles" . "']";
                    ...
                    if (!$this->readDataOnly) {
                        foreach ($xfTags as $xfTag) {
                            ...
                            $style = (object) ['numFmt' => $numFmt ?? NumberFormat::FORMAT_GENERAL, 'font' => $fonts[(int) $xf['fontId']], 'fill' => $fills[(int) $xf['fillId']], 'border' => $borders[(int) $xf['borderId']], 'alignment' => $xfTag->alignment, 'protection' => $xfTag->protection, 'quotePrefix' => $quotePrefix];
                            $styles[] = $style;
                        ...
                        foreach ($cellXfTags as $xfTag) {
                            ...
                            $quotePrefix = (bool) (string) ($xf['quotePrefix'] ?? '');
                            $cellStyle = (object) ['numFmt' => $numFmt, 'font' => $fonts[(int) $xf['fontId']], 'fill' => $fills[(int) $xf['fillId']], 'border' => $borders[(int) $xf['borderId']], 'alignment' => $xfTag->alignment, 'protection' => $xfTag->protection, 'quotePrefix' => $quotePrefix];
                    ...
                    $sharedStrings = [];
                    $relType = "rel:Relationship[@Type='" . "{$xmlNamespaceBase}/sharedStrings" . "']";
                    ...
                    if ($xmlWorkbookNS->sheets) {
                        ...
                        foreach ($xmlWorkbookNS->sheets->sheet as $eleSheet) {
                            ...
                            if ($xmlSheetNS) {
                                ...
                                (new ColumnAndRowAttributes($docSheet, $xmlSheetNS))->load($this->getReadFilter(), $this->getReadDataOnly());
                            }
                            if ($xmlSheetNS && $xmlSheetNS->sheetData && $xmlSheetNS->sheetData->row) {
                                ...
                                foreach ($xmlSheetNS->sheetData->row as $row) {
                                    ...
                                    foreach ($row->c as $c) {
                                        ...
                                        // Read cell!
                                        $useFormula = isset($c->f) && ((string) $c->f !== '' || isset($c->f->attributes()['t']) && \strtolower((string) $c->f->attributes()['t']) === 'shared');
                                        switch ($cellDataType) {
                                            case 's':
                                                if ((string) $c->v != '') {
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                foreach ($vmlComments as $relName => $relPath) {
                                    ...
                                    try {
                                        ...
                                        $vmlCommentsFile = $this->loadZip($relPath, '', \true);
                                        ...
                                        if (isset($shape['style'])) {
                                            ...
                                            if ($column !== null && $row !== null) {
                                                ...
                                                if (isset($drowingImages[$fillImageRelId])) {
                                                    $objDrawing = new \WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Drawing();
                                                    $objDrawing->setName($fillImageTitle);
                                                    $imagePath = \str_replace('../', 'xl/', $drowingImages[$fillImageRelId]);
                                                    $objDrawing->setPath('zip://' . File::realpath($filename) . '#' . $imagePath, \true, $zip);
                                        ...
                                        if ($vmlRelationship != '') {
                                            ...
                                            foreach ($shapes as $idx => $shape) {
                                                ...
                                                if (\array_key_exists((string) $imageData['relid'], $drawings)) {
                                                    ...
                                                    $hfImages[$shapeId]->setResizeProportional(\true);
                                                }
                                            }
                            ...
                            $drawingFilename = \dirname("{$dir}/{$fileWorksheet}") . '/_rels/' . \basename($fileWorksheet) . '.rels';
                                            ...
                                            foreach ($relsDrawing->Relationship as $ele) {
                                                ...
                                                if ($eleType === "{$xmlNamespaceBase}/image") {
                                                    ...
                                                    if ($this->includeCharts) {
                                                        ...
                                                        $charts[$index] = ['id' => (string) $ele['Id'], 'sheet' => $docSheet->getTitle()];
                                                    }
                                                }
                                            }
                                        }
                                        ...
                                        if ($xmlDrawingChildren->oneCellAnchor) {
                                            foreach ($xmlDrawingChildren->oneCellAnchor as $oneCellAnchor) {
                                                ...
                                                if ($oneCellAnchor->pic->blipFill) {
                                                    ...
                                                    $embedImageKey = (string) self::getArrayItem(self::getAttributes($blip, $xmlNamespaceBase), 'embed');
                                                    if (isset($images[$embedImageKey])) {
                                                        $objDrawing->setPath('zip://' . File::realpath($filename) . '#' . $images[$embedImageKey], \false, $zip);
                                                    } else {
                                                    ...
                                                    $chartDetails[$docSheet->getTitle() . '!' . $thisChart] = ['fromCoordinate' => $coordinates, 'fromOffsetX' => $offsetX, 'fromOffsetY' => $offsetY, 'width' => $width, 'height' => $height, 'worksheetTitle' => $docSheet->getTitle(), 'oneCellAnchor' => \true];
                                                }
                                            }
                                        }
                                        ...
                                        if ($xmlDrawingChildren->absoluteAnchor) {
                                            foreach ($xmlDrawingChildren->absoluteAnchor as $absoluteAnchor) {
                                                if ($this->includeCharts && $absoluteAnchor->graphicFrame) {
                                                    ...
                                                    $chartDetails[$docSheet->getTitle() . '!' . $thisChart] = ['fromCoordinate' => 'A1', 'fromOffsetX' => 0, 'fromOffsetY' => 0, 'width' => $width, 'height' => $height, 'worksheetTitle' => $docSheet->getTitle()];
                                                }
                                            }
                                        }
                                        ...
                                        switch ((string) $definedName['name']) {
                                            ...
                                            case '_xlnm.Print_Area':
                                                ...
                                                foreach ($rangeSets as $rangeSet) {
                                                    [, $rangeSet] = Worksheet::extractSheetTitle($rangeSet, \true);
                                                    if (empty($rangeSet)) {
                                                        continue;
                                                    }
                            ...
                            foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
                                ...
                                if ((string) $definedName['localSheetId'] != '') {
                                    ...
                                    switch ((string) $definedName['name']) {
                                        ...
                                        default:
                                            if ($mapSheetId[(int) $definedName['localSheetId']] !== null) {
                                                $range = Worksheet::extractSheetTitle((string) $definedName, \true);
                                                ...
                                                if (\strpos((string) $definedName, '!') !== \false) {
                                                    ...
                                                    if ($worksheet = $excel->getSheetByName($range[0])) {
                                                        // @phpstan-ignore-line
                                                        $excel->addDefinedName(DefinedName::createInstance((string) $definedName['name'], $worksheet, $extractedRange, \true, $scope));
                                                    } else {
                                                        $excel->addDefinedName(DefinedName::createInstance((string) $definedName['name'], $scope, $extractedRange, \true, $scope));
                                                    }
                                                } else {
                                                    $excel->addDefinedName(DefinedName::createInstance((string) $definedName['name'], $scope, $extractedRange, \true));
                                                }
                                            }
                                            break;
                                    }
                                    ...
                                    if (\strpos((string) $definedName, '!') !== \false) {
                                        ...
                                        // Extract sheet name
                                        [$extractedSheetName] = Worksheet::extractSheetTitle((string) $definedNameValueParts[0], \true);
                                        // @phpstan-ignore-line
                                        $extractedSheetName = \trim($extractedSheetName, "'");
                                        // Locate sheet
                ...
                foreach ($is->r as $run) {
                    if (!isset($run->rPr)) {
                        ...
                        if (isset($run->rPr->u)) {
                            ...
                            if (!isset($attr['val'])) {
                                $objFont->setUnderline(\WPDT\PhpOffice\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE);
                            } else {
    ...
    private function readRibbon(Spreadsheet $excel, string $customUITarget, ZipArchive $zip) : void
    {
        ...
        if ($dataRels) {
            ...
            $UIRels = \simplexml_load_string($this->getSecurityScannerOrThrow()->scan($dataRels), SimpleXMLElement::class, $this->parseHuge ? \LIBXML_PARSEHUGE : 0);
            if (\false !== $UIRels) {
     ...
     */
    private function readHyperLinkDrawing(\WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Drawing $objDrawing, SimpleXMLElement $cellAnchor, $hyperlinks) : void
    {
        ...
        $hyperlink = new Hyperlink($hyperlinks[$hlinkId], (string) self::getArrayItem(self::getAttributes($cellAnchor->pic->nvPicPr->cNvPr), 'name'));
    ...
    private function readProtection(Spreadsheet $excel, SimpleXMLElement $xmlWorkbook) : void
    {
        ...
        if ($xmlWorkbook->workbookProtection['revisionsPassword']) {
            $excel->getSecurity()->setRevisionsPassword((string) $xmlWorkbook->workbookProtection['revisionsPassword'], \true);
        }
        if ($xmlWorkbook->workbookProtection['workbookPassword']) {
            $excel->getSecurity()->setWorkbookPassword((string) $xmlWorkbook->workbookProtection['workbookPassword'], \true);
        }
    }
    ...
    private function readSheetProtection(Worksheet $docSheet, SimpleXMLElement $xmlSheet) : void
    {
        ...
        if ($algorithmName) {
            $protection->setPassword((string) $xmlSheet->sheetProtection['hashValue'], \true);
            ...
            $protection->setPassword((string) $xmlSheet->sheetProtection['password'], \true);
        }
                    ...
                    for ($row = $firstRow; $row <= $lastRow; ++$row) {
                        for ($col = $firstCol; $col !== $lastCol; ++$col) {
                            if ($numberStoredAsText === '1') {
                                $sheet->getCell("{$col}{$row}")->getIgnoredErrors()->setNumberStoredAsText(\true);
                            }
                            if ($formula === '1') {
                                $sheet->getCell("{$col}{$row}")->getIgnoredErrors()->setFormula(\true);
                            }
                            if ($twoDigitTextYear === '1') {
                                $sheet->getCell("{$col}{$row}")->getIgnoredErrors()->setTwoDigitTextYear(\true);
                            }
                            if ($evalError === '1') {
                                $sheet->getCell("{$col}{$row}")->getIgnoredErrors()->setEvalError(\true);
                            }
                        }
                    }
                }
