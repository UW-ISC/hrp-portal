...
namespace WPDT\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\Wizard;
...
abstract class WizardAbstract
{
    ...
    {
        return $this->style ?? new Style(\false, \true);
    }
    ...
    {
        if ($operandValueType === Wizard::VALUE_TYPE_LITERAL && \substr($operand, 0, 1) === '"' && \substr($operand, -1) === '"') {
    ...
    public static function reverseAdjustCellRef(string $condition, string $cellRange) : string
    {
        ...
        foreach ($splitCondition as &$value) {
            ...
            $i = $i === \false;
            if ($i) {
                $value = (string) \preg_replace_callback('/' . Calculation::CALCULATION_REGEXP_CELLREF_RELATIVE . '/i', function ($matches) use($referenceColumnIndex, $referenceRow) {
