...
use WPDT\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\CellStyleAssessor;
...
class Cell
{
    ...
    {
        return (string) NumberFormat::toFormattedString($this->getCalculatedValue(), (string) $this->getStyle()->getNumberFormat()->getFormatCode());
    }
    /**
    ...
    public function setValueExplicit($value, string $dataType = DataType::TYPE_STRING)
    {
        ...
        // @phpstan-ignore-line
        return $this->getParent()->get($cellCoordinate);
        // @phpstan-ignore-line
    }
    ...
    public static function setCalculateDateTimeType(int $calculateDateTimeType) : void
    {
        switch ($calculateDateTimeType) {
            ...
            case self::CALCULATE_TIME_FLOAT:
                ...
                break;
            default:
                throw new \WPDT\PhpOffice\PhpSpreadsheet\Calculation\Exception("Invalid value {$calculateDateTimeType} for calculated date time type");
        }
    }
    /**
    ...
    private function convertDateTimeInt($result)
    {
        if (\is_int($result)) {
            if (self::$calculateDateTimeType === self::CALCULATE_TIME_FLOAT) {
                ...
                if (SharedDate::isDateTime($this, $result, \true)) {
                    $result = (float) $result;
                }
            }
        }
     ...
     * @return mixed
     */
    public function getCalculatedValue(bool $resetLog = \true)
    {
        if ($this->dataType === DataType::TYPE_FORMULA) {
            try {
                ...
                throw new \WPDT\PhpOffice\PhpSpreadsheet\Calculation\Exception($this->getWorksheet()->getTitle() . '!' . $this->getCoordinate() . ' -> ' . $ex->getMessage(), $ex->getCode(), $ex);
            }
