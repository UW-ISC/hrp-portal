...
use WPDT\PhpOffice\PhpSpreadsheet\Style\ConditionalFormatting\ConditionalFormattingRuleExtension;
...
class Worksheet extends WriterPart
{
    ...
    private static function writeOtherCondElements(XMLWriter $objWriter, Conditional $conditional, string $cellCoordinate) : void
    {
        ...
        if ($conditional->getConditionType() == Conditional::CONDITION_CELLIS || $conditional->getConditionType() == Conditional::CONDITION_EXPRESSION || !empty($conditions)) {
    ...
    private function writeConditionalFormatting(XMLWriter $objWriter, PhpspreadsheetWorksheet $worksheet) : void
    {
        ...
        foreach ($worksheet->getConditionalStylesCollection() as $cellCoordinate => $conditionalStyles) {
            ...
            foreach ($conditionalStyles as $conditional) {
                ...
                if ($conditional->getConditionType() === Conditional::CONDITION_CONTAINSTEXT || $conditional->getConditionType() === Conditional::CONDITION_NOTCONTAINSTEXT || $conditional->getConditionType() === Conditional::CONDITION_BEGINSWITH || $conditional->getConditionType() === Conditional::CONDITION_ENDSWITH) {
    ...
    private function writeSheetData(XMLWriter $objWriter, PhpspreadsheetWorksheet $worksheet, array $stringTable) : void
    {
        ...
        while ($currentRow++ < $highestRow) {
            ...
            if ($isRowSet || $worksheet->rowDimensionExists($currentRow)) {
                ...
                // Write current row?
                $writeCurrentRow = $isRowSet || $rowDimension->getRowHeight() >= 0 || $rowDimension->getVisible() === \false || $rowDimension->getCollapsed() === \true || $rowDimension->getOutlineLevel() > 0 || $rowDimension->getXfIndex() !== null;
                if ($writeCurrentRow) {
                    // Start a new row
                    ...
                    // Row visibility
                    if (!$rowDimension->getVisible() === \true) {
                        $objWriter->writeAttribute('hidden', 'true');
                    }
                    // Collapsed
    ...
    private function writeCellInlineStr(XMLWriter $objWriter, string $mappedType, $cellValue) : void
    {
        ...
        if (!$cellValue instanceof RichText) {
            ...
            $objWriter->writeElement('t', StringHelper::controlCharacterPHP2OOXML(\htmlspecialchars($cellValue, Settings::htmlEntityFlags())));
            $objWriter->endElement();
        } else {
            $objWriter->startElement('is');
    ...
    private function writeCellFormula(XMLWriter $objWriter, string $cellValue, Cell $cell) : void
    {
        ...
        if (($attributes['t'] ?? null) === 'array') {
            ...
            self::writeElementIf($objWriter, $this->getParentWriter()->getOffice2003Compatibility() === \false, 'v', $this->getParentWriter()->getPreCalculateFormulas() && !\is_array($calculatedValue) && \substr($calculatedValue ?? '', 0, 1) !== '#' ? StringHelper::formatNumber($calculatedValue) : '0');
        }
    }
    /**
     * Write Cell.
