...
class SheetViewOptions extends BaseParserClass
{
    ...
    private function pageSetup(SimpleXMLElement $sheetPr) : void
    {
        if (isset($sheetPr->pageSetUpPr)) {
            ...
            if (isset($attr['fitToPage']) && !self::boolean((string) $attr['fitToPage'])) {
                ...
                $this->worksheet->getPageSetup()->setFitToPage(\true);
            }
        }
    }
    private function sheetFormat(SimpleXMLElement $sheetFormatPrx) : void
    {
        ...
        if (isset($sheetFormatPr['customHeight']) && self::boolean((string) $sheetFormatPr['customHeight']) && isset($sheetFormatPr['defaultRowHeight'])) {
            $this->worksheet->getDefaultRowDimension()->setRowHeight((float) $sheetFormatPr['defaultRowHeight']);
        }
        if (isset($sheetFormatPr['defaultColWidth'])) {
            $this->worksheet->getDefaultColumnDimension()->setWidth((float) $sheetFormatPr['defaultColWidth']);
        }
        if (isset($sheetFormatPr['zeroHeight']) && (string) $sheetFormatPr['zeroHeight'] === '1') {
            $this->worksheet->getDefaultRowDimension()->setZeroHeight(\true);
        }
    }
