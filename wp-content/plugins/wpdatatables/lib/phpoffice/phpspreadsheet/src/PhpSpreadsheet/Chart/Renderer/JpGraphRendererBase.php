...
use WPDT\AccBarPlot;
use WPDT\AccLinePlot;
use WPDT\BarPlot;
use WPDT\ContourPlot;
use WPDT\Graph;
use WPDT\GroupBarPlot;
use WPDT\LinePlot;
...
abstract class JpGraphRendererBase implements IRenderer
{
    ...
    public function __construct(Chart $chart)
    {
        ...
        $this->graph = null;
        $this->chart = $chart;
        self::$markSet = ['diamond' => MARK_DIAMOND, 'square' => MARK_SQUARE, 'triangle' => MARK_UTRIANGLE, 'x' => MARK_X, 'star' => MARK_STAR, 'dot' => MARK_FILLEDCIRCLE, 'dash' => MARK_DTRIANGLE, 'circle' => MARK_CIRCLE, 'plus' => MARK_CROSS];
    }
    /**
     * This method should be overriden in descendants to do real JpGraph library initialization.
     */
    protected static abstract function init() : void;
    ...
    private function renderAreaChart($groupCount) : void
    {
        ...
        for ($i = 0; $i < $groupCount; ++$i) {
            $this->renderPlotLine($i, \true, \false);
        }
    }
    ...
    private function renderCombinationChart($groupCount, $outputDestination)
    {
        ...
        for ($i = 0; $i < $groupCount; ++$i) {
            ...
            switch ($chartType) {
                ...
                case 'line3DChart':
                case 'lineChart':
                    $this->renderPlotLine($i, \false, \true);
                    break;
                case 'scatterChart':
                    $this->renderPlotScatter($i, \false);
                    break;
