...
namespace WPDT\PhpOffice\PhpSpreadsheet\Style\NumberFormat;
...
class NumberFormatter
{
    ...
    private static function mergeComplexNumberFormatMasks(array $numbers, array $masks) : array
    {
        ...
        return [\implode('.', $masks), \implode('.', \array_reverse($postDecimalMasks))];
    }
    /**
     * @param mixed $number
     */
     ...
     * @param mixed $number
     */
    private static function complexNumberFormatMask($number, string $mask, bool $splitOnPoint = \true) : string
    {
        /** @var float */
        $numberFloat = $number;
        if ($splitOnPoint) {
    ...
    private static function formatStraightNumericValue($value, string $format, array $matches, bool $useThousands) : string
    {
        ...
        if ($useThousands) {
            $value = \number_format($valueFloat, \strlen($right), StringHelper::getDecimalSeparator(), StringHelper::getThousandsSeparator());
    ...
    public static function format($value, string $format) : string
    {
        ...
        if (\strpos((string) $value, '0.') !== \false && (\strpos($baseFormat, '#.') !== \false || \strpos($baseFormat, '?.') !== \false)) {
