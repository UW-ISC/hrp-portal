...
use WPDT\PhpOffice\PhpSpreadsheet\Helper\Dimension;
...
use WPDT\PhpOffice\PhpSpreadsheet\Style\Style as CellStyle;
use WPDT\PhpOffice\PhpSpreadsheet\Worksheet\ColumnDimension;
use WPDT\PhpOffice\PhpSpreadsheet\Worksheet\RowDimension;
...
class Style
{
    ...
    private function writeFillStyle(Fill $fill) : void
    {
        switch ($fill->getFillType()) {
            case Fill::FILL_SOLID:
                $this->writer->writeAttribute('fo:background-color', \sprintf('#%s', \strtolower($fill->getStartColor()->getRGB())));
                break;
    ...
    protected function writeColumnProperties(ColumnDimension $columnDimension) : void
    {
        ...
        $this->writer->writeAttribute('style:column-width', \round($columnDimension->getWidth(Dimension::UOM_CENTIMETERS), 3) . 'cm');
    ...
    public function writeColumnStyles(ColumnDimension $columnDimension, int $sheetId) : void
    {
        ...
        $this->writer->writeAttribute('style:name', \sprintf('%s_%d_%d', self::COLUMN_STYLE_PREFIX, $sheetId, $columnDimension->getColumnNumeric()));
    ...
    protected function writeRowProperties(RowDimension $rowDimension) : void
    {
        ...
        $this->writer->writeAttribute('style:row-height', \round($rowDimension->getRowHeight(Dimension::UOM_CENTIMETERS), 3) . 'cm');
    ...
    public function writeRowStyles(RowDimension $rowDimension, int $sheetId) : void
    {
        ...
        $this->writer->writeAttribute('style:name', \sprintf('%s_%d_%d', self::ROW_STYLE_PREFIX, $sheetId, $rowDimension->getRowIndex()));
    ...
    public function writeTableStyle(Worksheet $worksheet, int $sheetId) : void
    {
        ...
        $this->writer->writeAttribute('style:name', \sprintf('%s%d', self::TABLE_STYLE_PREFIX, $sheetId));
        $this->writer->startElement('style:table-properties');
        $this->writer->writeAttribute('table:display', $worksheet->getSheetState() === Worksheet::SHEETSTATE_VISIBLE ? 'true' : 'false');
        $this->writer->endElement();
