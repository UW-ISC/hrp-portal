...
use WPDT\PhpOffice\PhpSpreadsheet\Calculation\Engine\BranchPruner;
use WPDT\PhpOffice\PhpSpreadsheet\Calculation\Engine\CyclicReferenceStack;
use WPDT\PhpOffice\PhpSpreadsheet\Calculation\Engine\Logger;
use WPDT\PhpOffice\PhpSpreadsheet\Calculation\Engine\Operands;
use WPDT\PhpOffice\PhpSpreadsheet\Calculation\Token\Stack;
...
use WPDT\PhpOffice\PhpSpreadsheet\ReferenceHelper;
use WPDT\PhpOffice\PhpSpreadsheet\Shared;
...
class Calculation
{
     ...
     * @var bool
     */
    private $calculationCacheEnabled = \true;
    /**
     * @var BranchPruner
     */
     ...
     * @var bool
     */
    private $branchPruningEnabled = \true;
    /**
     * List of operators that can be used within formulae
     * The true/false value indicates whether it is a binary operator or a unary operator.
     */
    private const CALCULATION_OPERATORS = ['+' => \true, '-' => \true, '*' => \true, '/' => \true, '^' => \true, '&' => \true, '%' => \false, '~' => \false, '>' => \true, '<' => \true, '=' => \true, '>=' => \true, '<=' => \true, '<>' => \true, '∩' => \true, '∪' => \true, ':' => \true];
    /**
     * List of binary operators (those that expect two operands).
     */
    private const BINARY_OPERATORS = ['+' => \true, '-' => \true, '*' => \true, '/' => \true, '^' => \true, '&' => \true, '>' => \true, '<' => \true, '=' => \true, '>=' => \true, '<=' => \true, '<>' => \true, '∩' => \true, '∪' => \true, ':' => \true];
    /**
     ...
     */
    private static $excelConstants = ['TRUE' => \true, 'FALSE' => \false, 'NULL' => null];
    ...
    {
        $this->setCalculationCacheEnabled(\true);
    }
    /**
    ...
    {
        $this->setBranchPruningEnabled(\true);
    }
    ...
    {
        $localeFileName = $localeDir . \str_replace('_', \DIRECTORY_SEPARATOR, $locale) . \DIRECTORY_SEPARATOR . $file;
    ...
    public function setLocale(string $locale)
    {
        ...
        if (\in_array($language, self::$validLocaleLanguages, \true)) {
     ...
     * @return mixed
     */
    public function calculateCellValue(?Cell $cell = null, $resetLog = \true)
    {
        if ($cell === null) {
            return null;
        }
        ...
        $this->cellStack[] = ['sheet' => $cell->getWorksheet()->getTitle(), 'cell' => $cell->getCoordinate()];
    ...
    public function _calculateFormulaValue($formula, $cellID = null, ?Cell $cell = null, bool $ignoreQuotePrefix = \false)
    {
        ...
        if ($cell !== null && $ignoreQuotePrefix === \false && $cell->getStyle()->getQuotePrefix() === \true) {
    ...
    private function showValue($value)
    {
        if ($this->debugLog->getWriteDebugLog()) {
            ...
            } elseif (\is_string($value) && \trim($value, self::FORMULA_STRING_QUOTE) == $value) {
                return self::FORMULA_STRING_QUOTE . $value . self::FORMULA_STRING_QUOTE;
     ...
     * @var array
     */
    private static $comparisonOperators = ['>' => \true, '<' => \true, '=' => \true, '>=' => \true, '<=' => \true, '<>' => \true];
    /**
     *    Operator Precedence.
    ...
    private function internalParseFormula($formula, ?Cell $cell = null)
    {
        ...
        while (\true) {
            ...
            if ($opCharacter === '-' && !$expectingOperator) {
                ...
                while ($stack->count() > 0 && ($o2 = $stack->last()) && isset(self::CALCULATION_OPERATORS[$o2['value']]) && @(self::$operatorAssociativity[$opCharacter] ? self::$operatorPrecedence[$opCharacter] < self::$operatorPrecedence[$o2['value']] : self::$operatorPrecedence[$opCharacter] <= self::$operatorPrecedence[$o2['value']])) {
                    $output[] = $stack->pop();
                ...
                if (\preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/miu', $val, $matches)) {
                    ...
                    if ($testPrevOp !== null && $testPrevOp['value'] === ':') {
                        ...
                        if (!\is_numeric($val) && (\ctype_alpha($val) === \false || \strlen($val) > 3) && \preg_match('/^' . self::CALCULATION_REGEXP_DEFINEDNAME . '$/mui', $val) !== \false && ($this->spreadsheet === null || $this->spreadsheet->getNamedRange($val) !== null)) {
                            ...
                            [$rangeWS1, $startRowColRef] = Worksheet::extractSheetTitle($startRowColRef, \true);
                            $rangeSheetRef = $rangeWS1;
                            if ($rangeWS1 !== '') {
                                $rangeWS1 .= '!';
                            }
                            $rangeSheetRef = \trim($rangeSheetRef, "'");
                            [$rangeWS2, $val] = Worksheet::extractSheetTitle($val, \true);
                            if ($rangeWS2 !== '') {
                                $rangeWS2 .= '!';
                            } else {
                                $rangeWS2 = $rangeWS1;
                    ...
                    } elseif (\preg_match('/^' . self::CALCULATION_REGEXP_ROW_RANGE . '/miu', \substr($formula, $index), $rowRangeReference)) {
                        ...
                        $stackItemReference = $val;
                    } elseif (\preg_match('/^' . self::CALCULATION_REGEXP_COLUMN_RANGE . '/miu', \substr($formula, $index), $columnRangeReference)) {
    ...
    private function processTokenStack($tokens, $cellID = null, ?Cell $cell = null)
    {
        ...
        foreach ($tokens as $tokenData) {
            ...
            if ($this->branchPruningEnabled && isset($tokenData['onlyIf'])) {
                ...
                $storeValueAsBool = $storeValue === null ? \true : (bool) Functions::flattenSingleValue($storeValue);
                if (\is_array($storeValue)) {
                    $wrappedItem = \end($storeValue);
                    $storeValue = \is_array($wrappedItem) ? \end($wrappedItem) : $wrappedItem;
                }
                if ((isset($storeValue) || $tokenData['reference'] === 'NULL') && (!$storeValueAsBool || Information\ErrorValue::isError($storeValue) || $storeValue === 'Pruned branch')) {
                    // If branching value is not true, we don't need to compute
                    if (!isset($fakedForBranchPruning['onlyIf-' . $onlyIfStoreKey])) {
                        $stack->push('Value', 'Pruned branch (only if ' . $onlyIfStoreKey . ') ' . $token);
                        $fakedForBranchPruning['onlyIf-' . $onlyIfStoreKey] = \true;
                    }
                    if (isset($storeKey)) {
                        // We are processing an if condition
                        // We cascade the pruning to the depending branches
                        $branchStore[$storeKey] = 'Pruned branch';
                        $fakedForBranchPruning['onlyIfNot-' . $storeKey] = \true;
                        $fakedForBranchPruning['onlyIf-' . $storeKey] = \true;
                    }
                    continue;
                }
            }
            if ($this->branchPruningEnabled && isset($tokenData['onlyIfNot'])) {
                ...
                if ((isset($storeValue) || $tokenData['reference'] === 'NULL') && ($storeValueAsBool || Information\ErrorValue::isError($storeValue) || $storeValue === 'Pruned branch')) {
                    ...
                    if (!isset($fakedForBranchPruning['onlyIfNot-' . $onlyIfNotStoreKey])) {
                        ...
                        $fakedForBranchPruning['onlyIfNot-' . $onlyIfNotStoreKey] = \true;
                    }
                    if (isset($storeKey)) {
                        // We are processing an if condition
                        // We cascade the pruning to the depending branches
                ...
                switch ($token) {
                    ...
                    case ':':
                        ...
                        if (\strpos($operand1Data['reference'] ?? '', '!') !== \false) {
                            [$sheet1, $operand1Data['reference']] = Worksheet::extractSheetTitle($operand1Data['reference'], \true);
                        } else {
                            $sheet1 = $pCellWorksheet !== null ? $pCellWorksheet->getTitle() : '';
                        }
                        [$sheet2, $operand2Data['reference']] = Worksheet::extractSheetTitle($operand2Data['reference'], \true);
                        if (empty($sheet2)) {
                            $sheet2 = $sheet1;
                        }
                        if (\trim($sheet1, "'") === \trim($sheet2, "'")) {
                            if ($operand1Data['reference'] === null && $cell !== null) {
                                if (\is_array($operand1Data['value'])) {
                                    $operand1Data['reference'] = $cell->getCoordinate();
                                } elseif (\trim($operand1Data['value']) != '' && \is_numeric($operand1Data['value'])) {
                                    $operand1Data['reference'] = $cell->getColumn() . $operand1Data['value'];
                                } elseif (\trim($operand1Data['value']) == '') {
                                    $operand1Data['reference'] = $cell->getCoordinate();
                                } else {
                            ...
                            if ($operand2Data['reference'] === null && $cell !== null) {
                                ...
                                } elseif (\trim($operand2Data['value']) != '' && \is_numeric($operand2Data['value'])) {
                                    $operand2Data['reference'] = $cell->getColumn() . $operand2Data['value'];
                                } elseif (\trim($operand2Data['value']) == '') {
                                    $operand2Data['reference'] = $cell->getCoordinate();
                                } else {
                        ...
                        if (\is_array($operand1) || \is_array($operand2)) {
                            ...
                            for ($row = 0; $row < $rows; ++$row) {
                                for ($column = 0; $column < $columns; ++$column) {
                                    $operand1[$row][$column] = Shared\StringHelper::substring(self::boolToString($operand1[$row][$column]) . self::boolToString($operand2[$row][$column]), 0, DataType::MAX_STRING_LENGTH);
                                }
                            }
                            $result = $operand1;
                        } else {
    ...
    private function executeArrayComparison($operand1, $operand2, $operation, Stack &$stack, bool $recursingArrays)
    {
        ...
        if (!\is_array($operand2)) {
            ...
            foreach ($operand1 as $x => $operandData) {
                ...
                $this->executeBinaryComparisonOperation($operandData, $operand2[$x], $operation, $stack, \true);
                $r = $stack->pop();
                $result[$x] = $r['value'];
            }
        }
    ...
    private function executeNumericBinaryOperation($operand1, $operand2, $operation, &$stack)
    {
        ...
        if ($this->validateBinaryOperand($operand1, $stack) === \false || $this->validateBinaryOperand($operand2, $stack) === \false) {
            return \false;
        }
     ...
     */
    public function extractCellRange(&$range = 'A1', ?Worksheet $worksheet = null, $resetLog = \true)
    {
        // Return value
        $returnValue = [];
        if ($worksheet !== null) {
            ...
            if (\strpos($range, '!') !== \false) {
                [$worksheetName, $range] = Worksheet::extractSheetTitle($range, \true);
     ...
     * @return mixed Array of values in range if range contains more than one element. Otherwise, a single value is returned.
     */
    public function extractNamedRange(string &$range = 'A1', ?Worksheet $worksheet = null, $resetLog = \true)
    {
        // Return value
        $returnValue = [];
        if ($worksheet !== null) {
    ...
    private function addDefaultArgumentValues(array $functionCall, array $args, array $emptyArguments) : array
    {
        ...
        if (\count($methodArguments) > 0) {
            ...
            foreach ($emptyArguments as $argumentId => $isArgumentEmpty) {
                if ($isArgumentEmpty === \true) {
                    $reflectedArgumentId = \count($args) - (int) $argumentId - 1;
                    if (!\array_key_exists($reflectedArgumentId, $methodArguments) || $methodArguments[$reflectedArgumentId]->isVariadic()) {
                        break;
                    }
    ...
    private function evaluateDefinedName(Cell $cell, DefinedName $namedRange, Worksheet $cellWorksheet, Stack $stack)
    {
        ...
        $recursiveCalculationCell = $definedNameWorksheet !== null && $definedNameWorksheet !== $cellWorksheet ? $definedNameWorksheet->getCell('A1') : $cell;
        $recursiveCalculationCellAddress = $recursiveCalculationCell->getCoordinate();
        // Adjust relative references in ranges and formulae so that we execute the calculation for the correct rows and columns
        $definedNameValue = ReferenceHelper::getInstance()->updateFormulaReferencesAnyWorksheet($definedNameValue, Coordinate::columnIndexFromString($cell->getColumn()) - 1, $cell->getRow() - 1);
        ...
        $result = $recursiveCalculator->_calculateFormulaValue($definedNameValue, $recursiveCalculationCellAddress, $recursiveCalculationCell, \true);
