...
class CellMatcher
{
    ...
    protected function setReferenceCellForExpressions(string $conditionalRange) : void
    {
        ...
        foreach ($conditionalRange as $rangeSet) {
            $absoluteRangeSet = \array_map([Coordinate::class, 'absoluteCoordinate'], $rangeSet);
    ...
    protected function conditionCellAdjustment(array $matches)
    {
        ...
        if (!empty($matches[4])) {
            $worksheet = $this->worksheet->getParentOrThrow()->getSheetByName(\trim($matches[4], "'"));
            if ($worksheet === null) {
                return $this->wrapValue(null);
            }
            return $this->wrapValue($worksheet->getCell(\str_replace('$', '', "{$column}{$row}"))->getCalculatedValue());
        }
        return $this->wrapValue($this->worksheet->getCell(\str_replace('$', '', "{$column}{$row}"))->getCalculatedValue());
    }
    protected function cellConditionCheck(string $condition) : string
    {
        ...
        foreach ($splitCondition as &$value) {
            ...
            $i = $i === \false;
            if ($i) {
                $value = (string) \preg_replace_callback('/' . Calculation::CALCULATION_REGEXP_CELLREF_RELATIVE . '/i', [$this, 'conditionCellAdjustment'], $value);
            }
        }
        unset($value);
    ...
    {
        return \array_map([$this, 'cellConditionCheck'], $conditions);
    }
    ...
    protected function processRangeOperator(Conditional $conditional) : bool
    {
        $conditions = $this->adjustConditionsForCellReferences($conditional->getConditions());
        \sort($conditions);
        $expression = \sprintf((string) \preg_replace('/\\bA1\\b/i', (string) $this->wrapCellValue(), self::COMPARISON_RANGE_OPERATORS[$conditional->getOperatorType()]), ...$conditions);
        return $this->evaluateExpression($expression);
    }
    protected function processDuplicatesComparison(Conditional $conditional) : bool
    {
        $worksheetName = $this->cell->getWorksheet()->getTitle();
        $expression = \sprintf(self::COMPARISON_DUPLICATES_OPERATORS[$conditional->getConditionType()], $worksheetName, $this->conditionalRange, $this->cellConditionCheck($this->cell->getCalculatedValue()));
        return $this->evaluateExpression($expression);
    }
    protected function processExpression(Conditional $conditional) : bool
    {
        ...
        $expression = (string) \preg_replace('/\\b' . $this->referenceCell . '\\b/i', (string) $this->wrapCellValue(), $expression);
        return $this->evaluateExpression($expression);
    }
