...
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xls\ConditionalFormatting;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xls\Style\CellFont;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Xls\Style\FillPattern;
use WPDT\PhpOffice\PhpSpreadsheet\RichText\RichText;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\CodePage;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\Date;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\Escher;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\Escher\DgContainer\SpgrContainer\SpContainer;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\Escher\DggContainer\BstoreContainer\BSE;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\File;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\OLE;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\OLERead;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\StringHelper;
use WPDT\PhpOffice\PhpSpreadsheet\Shared\Xls as SharedXls;
...
use WPDT\PhpOffice\PhpSpreadsheet\Worksheet\MemoryDrawing;
use WPDT\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup;
use WPDT\PhpOffice\PhpSpreadsheet\Worksheet\SheetView;
...
class Xls extends BaseReader
{
    ...
    const XLS_TYPE_CFHEADER = 0x1b0;
    const XLS_TYPE_CFRULE = 0x1b1;
        ...
        foreach ($this->definedname as $definedName) {
            if ($definedName['isBuiltInName']) {
                switch ($definedName['name']) {
                    case \pack('C', 0x6):
                        ...
                        foreach ($ranges as $range) {
                            ...
                            //        Foo!$C$7:$J$66
                            //        Bar!$A$1:$IV$2
                            $explodes = Worksheet::extractSheetTitle($range, \true);
                ...
                if (\strpos($definedName['formula'], '!') !== \false) {
                    $explodes = Worksheet::extractSheetTitle($definedName['formula'], \true);
                    if (($docSheet = $this->spreadsheet->getSheetByName($explodes[0])) || ($docSheet = $this->spreadsheet->getSheetByName(\trim($explodes[0], "'")))) {
                        $extractedRange = $explodes[1];
    ...
    private function readNote() : void
    {
        ...
        if ($this->version == self::XLS_BIFF8) {
            ...
            $this->cellNotes[$noteObjID] = ['cellRef' => $cellAddress, 'objectID' => $noteObjID, 'author' => $noteAuthor];
        } else {
            $extension = \false;
    ...
    private function readTextObject() : void
    {
        ...
        $this->textObjects[$this->textObjRef] = ['text' => $textStr, 'format' => \substr($text['recordData'], $text['spliceOffsets'][1], $cbRuns), 'alignment' => $grbitOpts, 'rotation' => $rot];
    }
    /**
     * Read BOF.
     */
    ...
    private function readFont() : void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $isItalic = (0x2 & self::getUInt2d($recordData, 2)) >> 1;
            if ($isItalic) {
                $objFont->setItalic(\true);
            }
            // bit: 2; mask 0x0004; underlined (redundant in BIFF5-BIFF8)
            // bit: 3; mask 0x0008; strikethrough
            $isStrike = (0x8 & self::getUInt2d($recordData, 2)) >> 3;
            if ($isStrike) {
    ...
    private function readXf() : void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                if ($bordersLeftStyle = Xls\Style\Border::lookup((0xf & self::getInt4d($recordData, 10)) >> 0)) {
                    $objStyle->getBorders()->getLeft()->setBorderStyle($bordersLeftStyle);
                }
                // bit: 7-4; mask: 0x000000F0; right style
                if ($bordersRightStyle = Xls\Style\Border::lookup((0xf0 & self::getInt4d($recordData, 10)) >> 4)) {
                    $objStyle->getBorders()->getRight()->setBorderStyle($bordersRightStyle);
                }
                // bit: 11-8; mask: 0x00000F00; top style
                if ($bordersTopStyle = Xls\Style\Border::lookup((0xf00 & self::getInt4d($recordData, 10)) >> 8)) {
                    $objStyle->getBorders()->getTop()->setBorderStyle($bordersTopStyle);
                }
                // bit: 15-12; mask: 0x0000F000; bottom style
                if ($bordersBottomStyle = Xls\Style\Border::lookup((0xf000 & self::getInt4d($recordData, 10)) >> 12)) {
                    $objStyle->getBorders()->getBottom()->setBorderStyle($bordersBottomStyle);
                }
                // bit: 22-16; mask: 0x007F0000; left color
                $objStyle->getBorders()->getLeft()->colorIndex = (0x7f0000 & self::getInt4d($recordData, 10)) >> 16;
                // bit: 29-23; mask: 0x3F800000; right color
                $objStyle->getBorders()->getRight()->colorIndex = (0x3f800000 & self::getInt4d($recordData, 10)) >> 23;
                // bit: 30; mask: 0x40000000; 1 = diagonal line from top left to right bottom
                $diagonalDown = (0x40000000 & self::getInt4d($recordData, 10)) >> 30 ? \true : \false;
                // bit: 31; mask: 0x80000000; 1 = diagonal line from bottom left to top right
                $diagonalUp = ((int) 0x80000000 & self::getInt4d($recordData, 10)) >> 31 ? \true : \false;
                if ($diagonalUp === \false) {
                ...
                $objStyle->getBorders()->getTop()->colorIndex = (0x7f & self::getInt4d($recordData, 14)) >> 0;
                // bit: 13-7; mask: 0x00003F80; bottom color
                $objStyle->getBorders()->getBottom()->colorIndex = (0x3f80 & self::getInt4d($recordData, 14)) >> 7;
                // bit: 20-14; mask: 0x001FC000; diagonal color
                $objStyle->getBorders()->getDiagonal()->colorIndex = (0x1fc000 & self::getInt4d($recordData, 14)) >> 14;
                // bit: 24-21; mask: 0x01E00000; diagonal style
                if ($bordersDiagonalStyle = Xls\Style\Border::lookup((0x1e00000 & self::getInt4d($recordData, 14)) >> 21)) {
                ...
                $objStyle->getFill()->startcolorIndex = (0x7f & self::getUInt2d($recordData, 18)) >> 0;
                ...
                $objStyle->getFill()->setFillType(Xls\Style\FillPattern::lookup((0x3f0000 & $borderAndBackground) >> 16));
                // bit: 24-22; mask: 0x01C00000; bottom line style
                $objStyle->getBorders()->getBottom()->setBorderStyle(Xls\Style\Border::lookup((0x1c00000 & $borderAndBackground) >> 22));
                // bit: 31-25; mask: 0xFE000000; bottom line color
                $objStyle->getBorders()->getBottom()->colorIndex = ((int) 0xfe000000 & $borderAndBackground) >> 25;
                // offset: 12; size: 4; cell border lines
                $borderLines = self::getInt4d($recordData, 12);
                // bit: 2-0; mask: 0x00000007; top line style
                $objStyle->getBorders()->getTop()->setBorderStyle(Xls\Style\Border::lookup((0x7 & $borderLines) >> 0));
                // bit: 5-3; mask: 0x00000038; left line style
                $objStyle->getBorders()->getLeft()->setBorderStyle(Xls\Style\Border::lookup((0x38 & $borderLines) >> 3));
                // bit: 8-6; mask: 0x000001C0; right line style
                $objStyle->getBorders()->getRight()->setBorderStyle(Xls\Style\Border::lookup((0x1c0 & $borderLines) >> 6));
    ...
    private function readSheet() : void
    {
        ...
        $this->sheets[] = ['name' => $rec_name, 'offset' => $rec_offset, 'sheetState' => $sheetState, 'sheetType' => $sheetType];
    }
    /**
    ...
    private function readExternalBook() : void
    {
        ...
        if (\strlen($recordData) > 4) {
            ...
            $this->externalBooks[] = ['type' => 'external', 'encodedUrl' => $encodedUrlString['value'], 'externalSheetNames' => $externalSheetNames];
        } elseif (\substr($recordData, 2, 2) == \pack('CC', 0x1, 0x4)) {
            // internal reference
            // offset: 0; size: 2; number of sheet in this document
            // offset: 2; size: 2; 0x01 0x04
            $this->externalBooks[] = ['type' => 'internal'];
        } elseif (\substr($recordData, 0, 4) == \pack('vCC', 0x1, 0x1, 0x3a)) {
            // add-in function
            // offset: 0; size: 2; 0x0001
            $this->externalBooks[] = ['type' => 'addInFunction'];
        } elseif (\substr($recordData, 0, 2) == \pack('v', 0x0)) {
            // DDE links, OLE links
            // offset: 0; size: 2; 0x0000
            // offset: 2; size: var; encoded source document name
            $this->externalBooks[] = ['type' => 'DDEorOLE'];
        }
    }
    /**
    ...
    private function readExternName() : void
    {
        ...
        if ($this->version == self::XLS_BIFF8) {
            ...
            $this->externalNames[] = ['name' => $nameString['value'], 'formula' => $formula];
        }
    }
    /**
    ...
    private function readDefinedName() : void
    {
        ...
        if ($this->version == self::XLS_BIFF8) {
            ...
            $this->definedname[] = ['isBuiltInName' => $isBuiltInName, 'name' => $string['value'], 'formula' => $formula, 'scope' => $scope];
        }
    }
    /**
    ...
    private function readSheetPr() : void
    {
        ...
        $isSummaryBelow = (0x40 & self::getUInt2d($recordData, 0)) >> 6;
        $this->phpSheet->setShowSummaryBelow((bool) $isSummaryBelow);
        // bit: 7; mask: 0x0080; 0 = outline buttons left of outline group
        $isSummaryRight = (0x80 & self::getUInt2d($recordData, 0)) >> 7;
        $this->phpSheet->setShowSummaryRight((bool) $isSummaryRight);
        // bit: 8; mask: 0x100; 0 = scale printout in percent, 1 = fit printout to number of pages
        // this corresponds to radio button setting in page setup dialog in Excel
        $this->isFitToPages = (bool) ((0x100 & self::getUInt2d($recordData, 0)) >> 8);
    }
    /**
    ...
    private function readPageSetup() : void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $isOverThenDown = 0x1 & self::getUInt2d($recordData, 10);
            // bit: 1; mask: 0x0002; 0=landscape, 1=portrait
            $isPortrait = (0x2 & self::getUInt2d($recordData, 10)) >> 1;
            // bit: 2; mask: 0x0004; 1= paper size, scaling factor, paper orient. not init
            // when this bit is set, do not use flags for those properties
            $isNotInit = (0x4 & self::getUInt2d($recordData, 10)) >> 2;
            if (!$isNotInit) {
    ...
    private function readProtect() : void
    {
        ...
        $bool = (0x1 & self::getUInt2d($recordData, 0)) >> 0;
    ...
    private function readColInfo() : void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $isHidden = (0x1 & self::getUInt2d($recordData, 8)) >> 0;
            // bit: 10-8; mask: 0x0700; outline level of the columns (0 = no outline)
            $level = (0x700 & self::getUInt2d($recordData, 8)) >> 8;
    ...
    private function readRow() : void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $level = (0x7 & self::getInt4d($recordData, 12)) >> 0;
            $this->phpSheet->getRowDimension($r + 1)->setOutlineLevel($level);
            // bit: 4; mask: 0x00000010; 1 = outline group start or ends here... and is collapsed
            $isCollapsed = (bool) ((0x10 & self::getInt4d($recordData, 12)) >> 4);
            $this->phpSheet->getRowDimension($r + 1)->setCollapsed($isCollapsed);
            // bit: 5; mask: 0x00000020; 1 = row is hidden
            $isHidden = (0x20 & self::getInt4d($recordData, 12)) >> 5;
            $this->phpSheet->getRowDimension($r + 1)->setVisible(!$isHidden);
            // bit: 7; mask: 0x00000080; 1 = row has explicit format
            $hasExplicitFormat = (0x80 & self::getInt4d($recordData, 12)) >> 7;
            // bit: 27-16; mask: 0x0FFF0000; only applies when hasExplicitFormat = 1; index to XF record
            $xfIndex = (0xfff0000 & self::getInt4d($recordData, 12)) >> 16;
    ...
    private function readObj() : void
    {
        ...
        $this->objs[] = ['ftCmoType' => $ftCmoType, 'cbCmoSize' => $cbCmoSize, 'otObjType' => $otObjType, 'idObjID' => $idObjID, 'grbitOpts' => $grbitOpts];
    ...
    private function readWindow2() : void
    {
        ...
        $this->phpSheet->setRightToLeft((bool) ((0x40 & $options) >> 6));
    ...
    private function readHyperLink() : void
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $isFileLinkOrUrl = (0x1 & self::getUInt2d($recordData, 28)) >> 0;
            // bit: 1; mask: 0x00000002; 0 = relative path, 1 = absolute path or URL
            $isAbsPathOrUrl = (0x1 & self::getUInt2d($recordData, 28)) >> 1;
            // bit: 2 (and 4); mask: 0x00000014; 0 = no description
            $hasDesc = (0x14 & self::getUInt2d($recordData, 28)) >> 2;
            // bit: 3; mask: 0x00000008; 0 = no text, 1 = has text
            $hasText = (0x8 & self::getUInt2d($recordData, 28)) >> 3;
            // bit: 7; mask: 0x00000080; 0 = no target frame, 1 = has target frame
            $hasFrame = (0x80 & self::getUInt2d($recordData, 28)) >> 7;
            // bit: 8; mask: 0x00000100; 0 = file link or URL, 1 = UNC path (inc. server name)
            $isUNC = (0x100 & self::getUInt2d($recordData, 28)) >> 8;
            // offset within record data
            $offset = 32;
            if ($hasDesc) {
            ...
            } elseif (\ord($recordData[$offset]) == 0x3) {
                $hyperlinkType = 'local';
    ...
    private function readBIFF8CellAddress($cellAddressStructure)
    {
        ...
        $column = Coordinate::stringFromColumnIndex((0xff & self::getUInt2d($cellAddressStructure, 2)) + 1);
    ...
    private function readBIFF8CellAddressB($cellAddressStructure, $baseCell = 'A1')
    {
        ...
        if (!(0x4000 & self::getUInt2d($cellAddressStructure, 2))) {
            // offset: 2; size: 2; index to column or column offset + relative flags
            // bit: 7-0; mask 0x00FF; column index
            $colIndex = 0xff & self::getUInt2d($cellAddressStructure, 2);
            $column = Coordinate::stringFromColumnIndex($colIndex + 1);
            $column = '$' . $column;
        } else {
            // offset: 2; size: 2; index to column or column offset + relative flags
            // bit: 7-0; mask 0x00FF; column index
            $relativeColIndex = 0xff & self::getInt2d($cellAddressStructure, 2);
    ...
    private function readBIFF8CellRangeAddress($subData)
    {
        ...
        // bit: 7-0; mask 0x00FF; column index
        $fc = Coordinate::stringFromColumnIndex((0xff & self::getUInt2d($subData, 4)) + 1);
        // bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
        ...
        // bit: 7-0; mask 0x00FF; column index
        $lc = Coordinate::stringFromColumnIndex((0xff & self::getUInt2d($subData, 6)) + 1);
        // bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
    ...
    private function readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
    {
        ...
        if (!(0x4000 & self::getUInt2d($subData, 4))) {
            ...
            // bit: 7-0; mask 0x00FF; column index
            $fcIndex = 0xff & self::getUInt2d($subData, 4);
            ...
            // offset: 4; size: 2; first column with relative/absolute flags
            // bit: 7-0; mask 0x00FF; column index
            $relativeFcIndex = 0xff & self::getInt2d($subData, 4);
        ...
        if (!(0x4000 & self::getUInt2d($subData, 6))) {
            ...
            // bit: 7-0; mask 0x00FF; column index
            $lcIndex = 0xff & self::getUInt2d($subData, 6);
            ...
            // offset: 4; size: 2; first column with relative/absolute flags
            // bit: 7-0; mask 0x00FF; column index
            $relativeLcIndex = 0xff & self::getInt2d($subData, 4);
    ...
    private function readCFHeader() : array
    {
        ...
        // offset: var; size: var; cell range address list with
        $cellRangeAddressList = $this->version == self::XLS_BIFF8 ? $this->readBIFF8CellRangeAddressList(\substr($recordData, 12)) : $this->readBIFF5CellRangeAddressList(\substr($recordData, 12));
        $cellRangeAddresses = $cellRangeAddressList['cellRangeAddresses'];
    ...
    private function readCFRule(array $cellRangeAddresses) : void
    {
        ...
        $style = new Style(\false, \true);
        // non-supervisor, conditional
        $this->getCFStyleOptions($options, $style);
        $hasFontRecord = (bool) ((0x4000000 & $options) >> 26);
        $hasAlignmentRecord = (bool) ((0x8000000 & $options) >> 27);
    ...
    private function getCFFontStyle(string $options, Style $style) : void
    {
        ...
        $bold = self::getUInt2d($options, 72) === 700;
        // 400 = normal, 700 = bold
