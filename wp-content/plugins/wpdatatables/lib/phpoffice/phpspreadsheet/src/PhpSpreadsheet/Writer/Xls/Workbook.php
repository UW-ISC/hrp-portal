...
class Workbook extends BIFFwriter
{
     ...
     */
    public function addFont(\WPDT\PhpOffice\PhpSpreadsheet\Style\Font $font)
    {
    ...
    private function addColor($rgb)
    {
        if (!isset($this->colors[$rgb])) {
            $color = [\hexdec(\substr($rgb, 0, 2)), \hexdec(\substr($rgb, 2, 2)), \hexdec(\substr($rgb, 4)), 0];
    ...
    private function parseDefinedNameValue(DefinedName $definedName) : string
    {
        ...
        $splitCount = \preg_match_all('/' . Calculation::CALCULATION_REGEXP_CELLREF . '/mui', $definedRange, $splitRanges, \PREG_OFFSET_CAPTURE);
    ...
    private function writeAllDefinedNamesBiff8() : string
    {
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
                ...
                $chunk .= $this->writeData($this->writeDefinedNameBiff8(\pack('C', 0x7), $formulaData, $i + 1, \true));
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                ...
                // store the DEFINEDNAME record
                $chunk .= $this->writeData($this->writeDefinedNameBiff8(\pack('C', 0x6), $formulaData, $i + 1, \true));
            }
        }
     ...
     */
    private function writeBoundSheet(\WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $sheet, $offset) : void
    {
        ...
        switch ($sheet->getSheetState()) {
            case \WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_VISIBLE:
                $ss = 0x0;
                break;
            case \WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_HIDDEN:
                $ss = 0x1;
                break;
            case \WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet::SHEETSTATE_VERYHIDDEN:
                $ss = 0x2;
                break;
            default:
                $ss = 0x0;
    ...
    private function writeSupbookInternal() : string
    {
        ...
        // Bytes to follow
        $header = \pack('vv', $record, $length);
        $data = \pack('vv', $this->spreadsheet->getSheetCount(), 0x401);
