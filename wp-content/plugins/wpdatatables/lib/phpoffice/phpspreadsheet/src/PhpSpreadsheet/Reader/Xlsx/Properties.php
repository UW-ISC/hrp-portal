...
class Properties
{
    ...
    private function extractPropertyData(string $propertyData) : ?SimpleXMLElement
    {
        ...
        $obj = \simplexml_load_string($this->securityScanner->scan($propertyData));
    ...
    public function readCoreProperties(string $propertyData) : void
    {
        ...
        if (\is_object($xmlCore)) {
            ...
            $this->docProps->setCreated((string) self::getArrayItem($xmlCore->xpath('dcterms:created')));
            //! respect xsi:type
            $this->docProps->setModified((string) self::getArrayItem($xmlCore->xpath('dcterms:modified')));
            //! respect xsi:type
