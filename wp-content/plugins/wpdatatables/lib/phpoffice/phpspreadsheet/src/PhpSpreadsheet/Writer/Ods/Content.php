...
use WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Row;
use WPDT\PhpOffice\PhpSpreadsheet\Worksheet\RowCellIterator;
use WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet;
use WPDT\PhpOffice\PhpSpreadsheet\Writer\Exception;
use WPDT\PhpOffice\PhpSpreadsheet\Writer\Ods;
use WPDT\PhpOffice\PhpSpreadsheet\Writer\Ods\Cell\Comment;
use WPDT\PhpOffice\PhpSpreadsheet\Writer\Ods\Cell\Style;
/**
    ...
    private function writeSheets(XMLWriter $objWriter) : void
    {
        ...
        for ($sheetIndex = 0; $sheetIndex < $sheetCount; ++$sheetIndex) {
            ...
            foreach ($spreadsheet->getSheet($sheetIndex)->getColumnDimensions() as $columnDimension) {
                ...
                $objWriter->startElement('table:table-column');
                $objWriter->writeAttribute('table:style-name', \sprintf('%s_%d_%d', Style::COLUMN_STYLE_PREFIX, $sheetIndex, $columnDimension->getColumnNumeric()));
            ...
            if ($cellIterator->valid()) {
                ...
                if ($span_row) {
                    ...
                    if ($sheet->getRowDimension($row->getRowIndex())->getRowHeight() > 0) {
                        $objWriter->writeAttribute('table:style-name', \sprintf('%s_%d_%d', Style::ROW_STYLE_PREFIX, $sheetIndex, $row->getRowIndex()));
                    }
