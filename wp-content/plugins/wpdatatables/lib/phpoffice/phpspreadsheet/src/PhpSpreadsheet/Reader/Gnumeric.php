...
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Gnumeric\PageSetup;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Gnumeric\Properties;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Gnumeric\Styles;
...
class Gnumeric extends BaseReader
{
    ...
    {
        return $xml->namespaceURI === self::NAMESPACE_GNM && $xml->localName === $expectedLocalName && $xml->nodeType === XMLReader::ELEMENT;
    }
    /**
    ...
    public function loadIntoExisting(string $filename, Spreadsheet $spreadsheet) : Spreadsheet
    {
        ...
        foreach ($gnmXML->Sheets->Sheet as $sheetOrNull) {
            ...
            if (\is_array($this->loadSheetsOnly) && !\in_array($worksheetName, $this->loadSheetsOnly, \true)) {
                continue;
            }
            $maxRow = $maxCol = 0;
            ...
            if (!$this->readDataOnly) {
                (new PageSetup($this->spreadsheet))->printInformation($sheet)->sheetMargins($sheet);
            }
    ...
    private function processDefinedNames(?SimpleXMLElement $gnmXML) : void
    {
        ...
        if ($gnmXML !== null && isset($gnmXML->Names)) {
            foreach ($gnmXML->Names->Name as $definedName) {
                ...
                [$worksheetName] = Worksheet::extractSheetTitle($value, \true);
                $worksheetName = \trim($worksheetName, "'");
    ...
    private function loadCell(SimpleXMLElement $cell, string $worksheetName, SimpleXMLElement $cellAttributes, string $column, int $row) : void
    {
        ...
        if (isset($cellAttributes->ValueFormat)) {
            $this->spreadsheet->getActiveSheet()->getCell($column . $row)->getStyle()->getNumberFormat()->setFormatCode((string) $cellAttributes->ValueFormat);
        }
    }
}
