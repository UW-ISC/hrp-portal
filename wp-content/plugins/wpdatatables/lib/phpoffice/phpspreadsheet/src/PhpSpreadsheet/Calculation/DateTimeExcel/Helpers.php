...
class Helpers
{
     ...
     */
    public static function getDateValue($dateValue, bool $allowBool = \true) : float
    {
    ...
    public static function returnIn3FormatsArray(array $dateArray, bool $noFrac = \false)
    {
        ...
        if ($retType === Functions::RETURNDATE_PHP_DATETIME_OBJECT) {
            return new DateTime($dateArray['year'] . '-' . $dateArray['month'] . '-' . $dateArray['day'] . ' ' . $dateArray['hour'] . ':' . $dateArray['minute'] . ':' . $dateArray['second']);
        }
        $excelDateValue = SharedDateHelper::formattedPHPToExcel($dateArray['year'], $dateArray['month'], $dateArray['day'], $dateArray['hour'], $dateArray['minute'], $dateArray['second']);
     ...
     * @param mixed $number
     */
    public static function nullFalseTrueToNumber(&$number, bool $allowBool = \true) : void
    {
