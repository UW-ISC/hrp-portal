...
use WPDT\PhpOffice\PhpSpreadsheet\Worksheet\BaseDrawing;
...
class Rels extends WriterPart
{
    ...
    public function writeRelationships(Spreadsheet $spreadsheet)
    {
        ...
        if (!empty($customPropertyList)) {
            ...
            $this->writeRelationship($objWriter, 4, Namespaces::RELATIONSHIPS_CUSTOM_PROPERTIES, 'docProps/custom.xml');
        }
        // Relationship docProps/app.xml
        $this->writeRelationship($objWriter, 3, Namespaces::RELATIONSHIPS_EXTENDED_PROPERTIES, 'docProps/app.xml');
        // Relationship docProps/core.xml
        $this->writeRelationship($objWriter, 2, Namespaces::CORE_PROPERTIES, 'docProps/core.xml');
        // Relationship xl/workbook.xml
        $this->writeRelationship($objWriter, 1, Namespaces::OFFICE_DOCUMENT, 'xl/workbook.xml');
        // a custom UI in workbook ?
        $target = $spreadsheet->getRibbonXMLData('target');
        if ($spreadsheet->hasRibbon()) {
            $this->writeRelationShip($objWriter, 5, Namespaces::EXTENSIBILITY, \is_string($target) ? $target : '');
        }
    ...
    public function writeWorkbookRelationships(Spreadsheet $spreadsheet)
    {
        ...
        $this->writeRelationship($objWriter, 1, Namespaces::STYLES, 'styles.xml');
        // Relationship theme/theme1.xml
        $this->writeRelationship($objWriter, 2, Namespaces::THEME2, 'theme/theme1.xml');
        // Relationship sharedStrings.xml
        $this->writeRelationship($objWriter, 3, Namespaces::SHARED_STRINGS, 'sharedStrings.xml');
        // Relationships with sheets
        $sheetCount = $spreadsheet->getSheetCount();
        for ($i = 0; $i < $sheetCount; ++$i) {
            $this->writeRelationship($objWriter, $i + 1 + 3, Namespaces::WORKSHEET, 'worksheets/sheet' . ($i + 1) . '.xml');
        }
        // Relationships for vbaProject if needed
        // id : just after the last sheet
        if ($spreadsheet->hasMacros()) {
            $this->writeRelationShip($objWriter, $i + 1 + 3, Namespaces::VBA, 'vbaProject.bin');
            ++$i;
     ...
     * @return string XML Output
     */
    public function writeWorksheetRelationships(\WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet, $worksheetId = 1, $includeCharts = \false, $tableRef = 1)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        if ($worksheet->getDrawingCollection()->count() > 0 || \count($charts) > 0 || $drawingOriginalIds) {
            ...
            $this->writeRelationship($objWriter, $rId, Namespaces::RELATIONSHIPS_DRAWING, $relPath);
        }
        // Write hyperlink relationships?
        $i = 1;
        foreach ($worksheet->getHyperlinkCollection() as $hyperlink) {
            if (!$hyperlink->isInternal()) {
                $this->writeRelationship($objWriter, '_hyperlink_' . $i, Namespaces::HYPERLINK, $hyperlink->getUrl(), 'External');
                ++$i;
            }
        }
        ...
        if (\count($worksheet->getComments()) > 0 || isset($unparsedLoadedData['sheets'][$worksheet->getCodeName()]['legacyDrawing'])) {
            $this->writeRelationship($objWriter, '_comments_vml' . $i, Namespaces::VML, '../drawings/vmlDrawing' . $worksheetId . '.vml');
        }
        if (\count($worksheet->getComments()) > 0) {
            $this->writeRelationship($objWriter, '_comments' . $i, Namespaces::COMMENTS, '../comments' . $worksheetId . '.xml');
        }
        // Write Table
        $tableCount = $worksheet->getTableCollection()->count();
        for ($i = 1; $i <= $tableCount; ++$i) {
            $this->writeRelationship($objWriter, '_table_' . $i, Namespaces::RELATIONSHIPS_TABLE, '../tables/table' . $tableRef++ . '.xml');
        }
        // Write header/footer relationship?
        $i = 1;
        if (\count($worksheet->getHeaderFooter()->getImages()) > 0) {
            $this->writeRelationship($objWriter, '_headerfooter_vml' . $i, Namespaces::VML, '../drawings/vmlDrawingHF' . $worksheetId . '.vml');
        }
    ...
    private function writeUnparsedRelationship(\WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet, XMLWriter $objWriter, string $relationship, string $type) : void
    {
        $unparsedLoadedData = $worksheet->getParentOrThrow()->getUnparsedLoadedData();
        ...
        foreach ($unparsedLoadedData['sheets'][$worksheet->getCodeName()][$relationship] as $rId => $value) {
            if (\substr($rId, 0, 17) !== '_headerfooter_vml') {
                $this->writeRelationship($objWriter, $rId, $type, $value['relFilePath']);
            }
        }
    }
    /**
     ...
     * @return string XML Output
     */
    public function writeDrawingRelationships(\WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet, &$chartRef, $includeCharts = \false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        while ($iterator->valid()) {
            ...
            if ($drawing instanceof \WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Drawing || $drawing instanceof MemoryDrawing) {
                // Write relationship for image drawing
                $this->writeRelationship($objWriter, $i, Namespaces::IMAGE, '../media/' . $drawing->getIndexedFilename());
        ...
        if ($includeCharts) {
            ...
            if ($chartCount > 0) {
                for ($c = 0; $c < $chartCount; ++$c) {
                    $this->writeRelationship($objWriter, $i++, Namespaces::RELATIONSHIPS_CHART, '../charts/chart' . ++$chartRef . '.xml');
                }
            }
        }
        $objWriter->endElement();
     ...
     * @return string XML Output
     */
    public function writeHeaderFooterDrawingRelationships(\WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        foreach ($worksheet->getHeaderFooter()->getImages() as $key => $value) {
            // Write relationship for image drawing
            $this->writeRelationship($objWriter, $key, Namespaces::IMAGE, '../media/' . $value->getIndexedFilename());
        }
        $objWriter->endElement();
        return $objWriter->getData();
    }
    public function writeVMLDrawingRelationships(\WPDT\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $worksheet) : string
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        foreach ($worksheet->getComments() as $comment) {
            ...
            $this->writeRelationship($objWriter, $bgImage->getImageIndex(), Namespaces::IMAGE, '../media/' . $bgImage->getMediaFilename());
        }
        $objWriter->endElement();
        return $objWriter->getData();
    }
    ...
    private function writeDrawingHyperLink(XMLWriter $objWriter, BaseDrawing $drawing, int $i) : int
    {
        ...
        ++$i;
        $this->writeRelationship($objWriter, $i, Namespaces::HYPERLINK, $drawing->getHyperlink()->getUrl(), $drawing->getHyperlink()->getTypeHyperlink());
        return $i;
    }
}
