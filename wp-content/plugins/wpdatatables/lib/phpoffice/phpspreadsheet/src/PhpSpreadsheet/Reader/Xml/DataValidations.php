...
class DataValidations
{
    ...
    public function loadDataValidations(SimpleXMLElement $worksheet, Spreadsheet $spreadsheet) : void
    {
        ...
        foreach ($xmlX->DataValidation as $dataValidation) {
            ...
            // set defaults
            $validation->setShowDropDown(\true);
            $validation->setShowInputMessage(\true);
            $validation->setShowErrorMessage(\true);
            $validation->setShowDropDown(\true);
            $this->thisRow = 1;
            $this->thisColumn = 1;
                    ...
                    case 'Range':
                        foreach (\explode(',', $tagValue) as $range) {
                            ...
                            if (\preg_match('/^R(\\d+)C(\\d+):R(\\d+)C(\\d+)$/', (string) $range, $selectionMatches) === 1) {
                                // range
                                $firstCell = Coordinate::stringFromColumnIndex((int) $selectionMatches[2]) . $selectionMatches[1];
                                $cell = $firstCell . ':' . Coordinate::stringFromColumnIndex((int) $selectionMatches[4]) . $selectionMatches[3];
                                ...
                                // cell
                                $cell = Coordinate::stringFromColumnIndex((int) $selectionMatches[2]) . $selectionMatches[1];
                                $sheet->getCell($cell);
                                $this->thisRow = (int) $selectionMatches[1];
                                $this->thisColumn = (int) $selectionMatches[2];
                            } elseif (\preg_match('/^C(\\d+)$/', (string) $range, $selectionMatches) === 1) {
                                // column
                                $firstCell = Coordinate::stringFromColumnIndex((int) $selectionMatches[1]) . '1';
                                $cell = $firstCell . ':' . Coordinate::stringFromColumnIndex((int) $selectionMatches[1]) . (string) AddressRange::MAX_ROW;
                                $this->thisColumn = (int) $selectionMatches[1];
                                $sheet->getCell($firstCell);
                            } elseif (\preg_match('/^R(\\d+)$/', (string) $range, $selectionMatches)) {
                                // row
                                $firstCell = 'A' . $selectionMatches[1];
                                $cell = $firstCell . ':' . AddressRange::MAX_COLUMN . $selectionMatches[1];
                                $this->thisRow = (int) $selectionMatches[1];
                                $sheet->getCell($firstCell);
                            }
