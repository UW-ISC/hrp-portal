...
use WPDT\PhpOffice\PhpSpreadsheet\Helper\Dimension as HelperDimension;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Ods\AutoFilter;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Ods\DefinedNames;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Ods\FormulaTranslator;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Ods\PageSettings;
use WPDT\PhpOffice\PhpSpreadsheet\Reader\Ods\Properties as DocumentProperties;
...
class Ods extends BaseReader
{
        ...
        if (File::testFileNoThrow($filename, '')) {
            ...
            if ($zip->open($filename) === \true) {
                ...
                if (!empty($stat) && $stat['size'] <= 255) {
                    ...
                    $xml = \simplexml_load_string($this->getSecurityScannerOrThrow()->scan($zip->getFromName('META-INF/manifest.xml')));
                    if ($xml !== \false) {
    ...
    public function listWorksheetNames($filename)
    {
        ...
        $worksheetNames = [];
        $xml = new XMLReader();
        $xml->xml($this->getSecurityScannerOrThrow()->scanFile('zip://' . \realpath($filename) . '#' . self::INITIAL_FILE));
    ...
    public function loadIntoExisting($filename, Spreadsheet $spreadsheet)
    {
        ...
        $zip->open($filename);
        // Meta
        $xml = @\simplexml_load_string($this->getSecurityScannerOrThrow()->scan($zip->getFromName('meta.xml')));
        if ($xml === \false) {
        ...
        $dom->loadXML($this->getSecurityScannerOrThrow()->scan($zip->getFromName('styles.xml')));
        $pageSettings = new PageSettings($dom);
        // Main Content
        $dom = new DOMDocument('1.01', 'UTF-8');
        $dom->loadXML($this->getSecurityScannerOrThrow()->scan($zip->getFromName(self::INITIAL_FILE)));
        ...
        foreach ($spreadsheets as $workbookData) {
            ...
            foreach ($tables as $worksheetDataSet) {
                ...
                if ($this->loadSheetsOnly !== null && $worksheetName && !\in_array($worksheetName, $this->loadSheetsOnly)) {
                    continue;
                }
                ...
                if ($worksheetID > 0) {
                    ...
                    // First sheet is added by default
                }
                        ...
                        case 'table-column':
                            ...
                            if (isset($columnWidths[$tableStyleName])) {
                                ...
                                for ($rowRepeats2 = $rowRepeats; $rowRepeats2 > 0; --$rowRepeats2) {
                                    $spreadsheet->getActiveSheet()->getColumnDimension($tableColumnString)->setWidth($columnWidth->toUnit('cm'), 'cm');
                                    ++$tableColumnString;
                                }
                            }
                                    ...
                                    switch ($type) {
                                        ...
                                        case 'date':
                                            ...
                                            if ($dataValue != \floor($dataValue)) {
                                                $formatting = NumberFormat::FORMAT_DATE_XLSX15 . ' ' . NumberFormat::FORMAT_DATE_TIME4;
                                            } else {
                                        ...
                                        case 'time':
                                            ...
                                            $dataValue = Date::PHPToExcel(\strtotime('01-01-1970 ' . \implode(
                                                ':',
                                                /** @scrutinizer ignore-type */
                                                \sscanf($timeValue, 'PT%dH%dM%dS') ?? []
                                            )));
                                    ...
                                    for ($i = 0; $i < $colRepeats; ++$i) {
                                        ...
                                        if ($type !== DataType::TYPE_NULL) {
                                            for ($rowAdjust = 0; $rowAdjust < $rowRepeats; ++$rowAdjust) {
                                                $rID = $rowID + $rowAdjust;
                                                $cell = $spreadsheet->getActiveSheet()->getCell($columnID . $rID);
                                                // Set value
                                                if ($hasCalculatedValue) {
                                                ...
                                                if ($formatting !== null) {
                                                    ...
                                                    $spreadsheet->getActiveSheet()->getStyle($columnID . $rID)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_GENERAL);
                                                }
                                                if ($hyperlink !== null) {
                                                    $cell->getHyperlink()->setUrl($hyperlink);
                                                }
                                            }
                                        }
                                    }
    ...
    {
        $dom = new DOMDocument('1.01', 'UTF-8');
        $dom->loadXML($this->getSecurityScannerOrThrow()->scan($zip->getFromName('settings.xml')));
        //$xlinkNs = $dom->lookupNamespaceUri('xlink');
        $configNs = $dom->lookupNamespaceUri('config');
        //$oooNs = $dom->lookupNamespaceUri('ooo');
        $officeNs = $dom->lookupNamespaceUri('office');
        $settings = $dom->getElementsByTagNameNS($officeNs, 'settings')->item(0);
        if ($settings !== null) {
    ...
    {
        if ($cellData->hasAttributeNS($tableNs, 'number-columns-spanned') || $cellData->hasAttributeNS($tableNs, 'number-rows-spanned')) {
