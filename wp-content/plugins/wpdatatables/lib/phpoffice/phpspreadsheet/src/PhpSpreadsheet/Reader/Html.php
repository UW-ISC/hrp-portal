...
use WPDT\PhpOffice\PhpSpreadsheet\Helper\Dimension as CssDimension;
...
class Html extends BaseReader
{
     ...
     */
    protected $formats = [
        'h1' => ['font' => ['bold' => \true, 'size' => 24]],
        //    Bold, 24pt
        'h2' => ['font' => ['bold' => \true, 'size' => 18]],
        //    Bold, 18pt
        'h3' => ['font' => ['bold' => \true, 'size' => 13.5]],
        //    Bold, 13.5pt
        'h4' => ['font' => ['bold' => \true, 'size' => 12]],
        //    Bold, 12pt
        'h5' => ['font' => ['bold' => \true, 'size' => 10]],
        //    Bold, 10pt
        'h6' => ['font' => ['bold' => \true, 'size' => 7.5]],
        //    Bold, 7.5pt
        'a' => ['font' => ['underline' => \true, 'color' => ['argb' => Color::COLOR_BLUE]]],
        //    Blue underlined
        'hr' => ['borders' => ['bottom' => ['borderStyle' => Border::BORDER_THIN, 'color' => [Color::COLOR_BLACK]]]],
        //    Bottom border
        'strong' => ['font' => ['bold' => \true]],
        //    Bold
        'b' => ['font' => ['bold' => \true]],
        //    Bold
        'i' => ['font' => ['italic' => \true]],
                ...
                if (isset($attributeArray['data-type'])) {
                    ...
                    if (\in_array($datatype, [DataType::TYPE_STRING, DataType::TYPE_STRING2, DataType::TYPE_INLINE])) {
                        ...
                        if (\substr($cellContent, 0, 1) === '=') {
                            $sheet->getCell($column . $row)->getStyle()->setQuotePrefix(\true);
                        }
                    }
                    ...
                    } catch (\WPDT\PhpOffice\PhpSpreadsheet\Exception $exception) {
    ...
    private function processDomElementTitle(Worksheet $sheet, int &$row, string &$column, string &$cellContent, DOMElement $child, array &$attributeArray) : void
    {
        if ($child->nodeName === 'title') {
            $this->processDomElement($child, $sheet, $row, $column, $cellContent);
            $sheet->setTitle($cellContent, \true, \true);
            $cellContent = '';
        } else {
    ...
    {
        if (\in_array((string) $child->nodeName, self::SPAN_ETC, \true)) {
            if (isset($attributeArray['class']) && $attributeArray['class'] === 'comment') {
                $sheet->getComment($column . $row)->getText()->createTextRun($child->textContent);
            } else {
                $this->processDomElement($child, $sheet, $row, $column, $cellContent);
            }
    ...
    private function processDomElementBr(Worksheet $sheet, int &$row, string &$column, string &$cellContent, DOMElement $child, array &$attributeArray) : void
    {
        if ($child->nodeName === 'br' || $child->nodeName === 'hr') {
            if ($this->tableLevel > 0) {
                ...
                $cellContent .= "\n";
                $sheet->getStyle($column . $row)->getAlignment()->setWrapText(\true);
            } else {
    ...
    {
        if (\in_array((string) $child->nodeName, self::H1_ETC, \true)) {
            if ($this->tableLevel > 0) {
                //    If we're inside a table, replace with a \n
    ...
    private function processDomElementBgcolor(Worksheet $sheet, int $row, string $column, array $attributeArray) : void
    {
        if (isset($attributeArray['bgcolor'])) {
            $sheet->getStyle("{$column}{$row}")->applyFromArray(['fill' => ['fillType' => Fill::FILL_SOLID, 'color' => ['rgb' => $this->getStyleColor($attributeArray['bgcolor'])]]]);
        }
    }
        ...
        foreach ($styles as $st) {
            ...
            switch ($styleName) {
                ...
                case 'border-right':
                    ...
                    break;
                case 'font-size':
                    $cellStyle->getFont()->setSize((float) $styleValue);
                    break;
                case 'font-weight':
                    if ($styleValue === 'bold' || $styleValue >= 500) {
                        $cellStyle->getFont()->setBold(\true);
                    }
                    break;
                case 'font-style':
                    if ($styleValue === 'italic') {
                        $cellStyle->getFont()->setItalic(\true);
                    }
                    break;
                case 'font-family':
                ...
                case 'text-decoration':
                    switch ($styleValue) {
                        case 'underline':
                            ...
                            break;
                        case 'line-through':
                            $cellStyle->getFont()->setStrikethrough(\true);
                            break;
                    }
                    break;
                case 'text-align':
                ...
                case 'width':
                    if ($column !== '') {
                        $sheet->getColumnDimension($column)->setWidth((new CssDimension($styleValue ?? ''))->width());
                    }
                    break;
                case 'height':
                    if ($row > 0) {
                        $sheet->getRowDimension($row)->setRowHeight((new CssDimension($styleValue ?? ''))->height());
                    }
                    break;
                case 'word-wrap':
                    $cellStyle->getAlignment()->setWrapText($styleValue === 'break-word');
                    break;
                case 'text-indent':
                    $cellStyle->getAlignment()->setIndent((int) \str_replace(['px'], '', $styleValueString));
                    break;
            }
        }
    }
    ...
    public function getStyleColor($value)
    {
        ...
        return \WPDT\PhpOffice\PhpSpreadsheet\Helper\Html::colourNameLookup($value);
    }
    /**
     * @param string    $column
     * @param int       $row
    ...
    private function insertImage(Worksheet $sheet, $column, $row, array $attributes) : void
    {
        ...
        $drawing->setResizeProportional(\true);
        if ($name) {
        ...
        $sheet->getColumnDimension($column)->setWidth($drawing->getWidth() / 6);
        $sheet->getRowDimension($row)->setRowHeight($drawing->getHeight() * 0.9);
    }
    ...
    private function setBorderStyle(Style $cellStyle, $styleValue, $type) : void
    {
        ...
        $cellStyle->applyFromArray(['borders' => [$type => ['borderStyle' => $this->getBorderStyle($borderStyle), 'color' => ['rgb' => $this->getStyleColor($color)]]]]);
    }
}
