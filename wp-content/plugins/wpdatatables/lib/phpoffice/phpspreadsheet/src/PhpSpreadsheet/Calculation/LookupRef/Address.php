...
class Address
{
    ...
    public const REFERENCE_STYLE_A1 = \true;
     ...
     */
    public static function cell($row, $column, $relativity = 1, $referenceStyle = \true, $sheetName = '')
    {
        if (\is_array($row) || \is_array($column) || \is_array($relativity) || \is_array($referenceStyle) || \is_array($sheetName)) {
            return self::evaluateArrayArguments([self::class, __FUNCTION__], $row, $column, $relativity, $referenceStyle, $sheetName);
        }
        $relativity = $relativity ?? 1;
        $referenceStyle = $referenceStyle ?? \true;
        if ($row < 1 || $column < 1) {
