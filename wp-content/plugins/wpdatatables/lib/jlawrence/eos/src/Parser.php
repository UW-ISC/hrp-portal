...
class Parser
{
    ...
    protected static function replaceVars($infix, $vArray)
    {
        ...
        while (\preg_match('/([^a-zA-Z]){0,1}([a-zA-Z]+)([^a-zA-Z]){0,1}/', $infix, $match) != 0) {
            ...
            if (!\in_array($match[1], \array_merge(self::$ST, self::$ST1, self::$ST2, self::$SEP['open'])) && $match[1] != "" || \is_numeric($match[1])) {
                //$this->SEP['close'] removed
                $front = "*";
            } else {
                $front = "";
            }
            if (!\in_array($match[3], \array_merge(self::$ST, self::$ST1, self::$ST2, self::$SEP['close'])) && $match[3] != "" || \is_numeric($match[3])) {
    ...
    {
        // Set up coefficients
        $p = array(0 => 1.000000000190015, 1 => 76.18009172947146, 2 => -86.50532032941678, 3 => 24.01409824083091, 4 => -1.231739572450155, 5 => 0.001208650973866179, 6 => -5.395239384953E-6);
        // Formula:
