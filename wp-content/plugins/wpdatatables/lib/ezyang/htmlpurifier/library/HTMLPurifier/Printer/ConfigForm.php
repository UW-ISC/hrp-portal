...
class HTMLPurifier_Printer_ConfigForm_default extends \WPDT\HTMLPurifier_Printer
{
    ...
    public function render($ns, $directive, $value, $name, $config)
    {
        ...
        $attr = array('name' => "{$name}" . "[{$ns}.{$directive}]", 'id' => "{$name}:{$ns}.{$directive}");
        if ($value === null) {
...
class HTMLPurifier_Printer_ConfigForm_bool extends \WPDT\HTMLPurifier_Printer
{
    ...
    public function render($ns, $directive, $value, $name, $config)
    {
        ...
        $ret .= $this->text(' Yes');
        $ret .= $this->end('label');
        $attr = array('type' => 'radio', 'name' => "{$name}" . "[{$ns}.{$directive}]", 'id' => "{$name}:Yes_{$ns}.{$directive}", 'value' => '1');
        if ($value === \true) {
            $attr['checked'] = 'checked';
        }
        if ($value === null) {
        ...
        $ret .= $this->text(' No');
        $ret .= $this->end('label');
        $attr = array('type' => 'radio', 'name' => "{$name}" . "[{$ns}.{$directive}]", 'id' => "{$name}:No_{$ns}.{$directive}", 'value' => '0');
        if ($value === \false) {
            $attr['checked'] = 'checked';
        }
        if ($value === null) {
