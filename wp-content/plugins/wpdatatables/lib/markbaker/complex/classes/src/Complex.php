...
class Complex
{
    ...
    private static function parseComplex($complexNumber)
    {
        ...
        $validComplex = \preg_match(self::NUMBER_SPLIT_REGEXP, $complexNumber, $complexParts);
        if (!$validComplex) {
    ...
    {
        return new Complex($this->imaginaryPart, $this->realPart, $this->realPart == 0.0 ? null : $this->suffix);
    }
    public function invertImaginary() : Complex
    {
        return new Complex($this->realPart, $this->imaginaryPart * -1, $this->imaginaryPart == 0.0 ? null : $this->suffix);
    }
    public function invertReal() : Complex
    {
        return new Complex($this->realPart * -1, $this->imaginaryPart, $this->imaginaryPart == 0.0 ? null : $this->suffix);
    }
    ...
    public function __call($functionName, $arguments)
    {
        ...
        if (\in_array($functionName, self::$functions, \true)) {
            return Functions::$functionName($this, ...$arguments);
        }
        // Test for operation calls
        if (\in_array($functionName, self::$operations, \true)) {
