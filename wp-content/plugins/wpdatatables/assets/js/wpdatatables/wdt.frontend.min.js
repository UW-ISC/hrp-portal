...
search+=buildSearchStringForMultiFilters(value,aoColumn.exactFiltering)});oTable.fnFilter(search.substring(0,search.length-1),columnIndex,!0,!1)}
if(typeof wpDataTables[tableId].drawTable==='undefined'||wpDataTables[tableId].drawTable===!0){oTable.api().draw()}
fnOnFiltered()}}
...
if(tableDescription.tableSkin==='raspberry-cream'){dataTableOptions.fnInitComplete=function(){let columnPos=tableDescription.dataTableParams.order[0][0];let columnTitle=tableDescription.dataTableParams.columnDefs[columnPos].className.substring(tableDescription.dataTableParams.columnDefs[columnPos].className.indexOf("column-")+7,);let tableId=tableDescription.tableId;addOutlineBorder(tableId,columnTitle);if(tableDescription.hideBeforeLoad){$(tableDescription.selector).animateFadeIn()}}}
