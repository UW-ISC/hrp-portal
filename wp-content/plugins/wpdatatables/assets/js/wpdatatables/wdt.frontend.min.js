...
wdtClearFilters()})}})(jQuery);var sRangeFormat=wpdatatables_filter_strings.from_columnfilter+" {from} "+wpdatatables_filter_strings.to_columnfilter+" {to}";var fnOnFiltered=function(){};function buildSafeFilterOption(value,text,selected,encodeOptionValue){if(encodeOptionValue===undefined){encodeOptionValue=!0}
var optionValue=encodeOptionValue?wdtEncodeFilterOptionValue(value):(value==null?'':String(value));var $option=jQuery('<option></option>');$option.attr('value',optionValue).text(text!==undefined&&text!==null?text:value);if(selected){$option.prop('selected',!0)}
return $option}
function buildSafeFilterOptionHtml(value,text,selected,encodeOptionValue){return buildSafeFilterOption(value,text,selected,encodeOptionValue)[0].outerHTML}
function wdtEncodeFilterOptionValue(value){return encodeURI(value==null?'':String(value))}
function wdtDecodeFilterOptionValue(value){if(value==null||value===''){return''}
var str=String(value).replace(/%0D%0A|%0A/gi,' ');try{return decodeURIComponent(str)}catch(e){return str}}
function wdtEscapeFilterSearchRegex(value){if(typeof jQuery!=='undefined'&&jQuery.fn&&jQuery.fn.dataTable&&jQuery.fn.dataTable.util&&typeof jQuery.fn.dataTable.util.escapeRegex==='function'){return jQuery.fn.dataTable.util.escapeRegex(String(value))}
return String(value).replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}
function wdtNormalizeRawFilterValueForSearch(value){if(value!=null&&typeof value==='object'&&value.value!=null){return wdtEncodeFilterOptionValue(value.value)}
return wdtEncodeFilterOptionValue(value)}
function wdtDecodeHtmlEntities(value){var str=String(value==null?'':value);if(str.indexOf('&')===-1){return str.trim()}
var namedEntities={amp:'&',lt:'<',gt:'>',quot:'"',apos:"'",nbsp:'\u00a0'};return str.replace(/&(#(?:x[0-9a-fA-F]+|\d+)|[a-zA-Z]+);/g,function(match,entity){if(entity.charAt(0)==='#'){var codePoint=entity.charAt(1).toLowerCase()==='x'?parseInt(entity.slice(2),16):parseInt(entity.slice(1),10);if(isNaN(codePoint)){return match}
return String.fromCharCode(codePoint)}
if(Object.prototype.hasOwnProperty.call(namedEntities,entity)){return namedEntities[entity]}
return match}).trim()}
function wdtFilterDefaultMatchesOptionValue(optionValue,defaultValue){if(defaultValue===''||defaultValue==null){return!1}
var defaultCompareValue=defaultValue;if(typeof defaultValue==='object'){defaultCompareValue=defaultValue.value!=null?defaultValue.value:defaultValue.value}
if(optionValue==defaultCompareValue){return!0}
return wdtDecodeHtmlEntities(optionValue)===wdtDecodeHtmlEntities(defaultCompareValue)}
function wdtSyncSelectFilterDefaultValue($select,defaultValue,selectedEncodedValues){if(selectedEncodedValues&&selectedEncodedValues.length){$select.val($select.prop('multiple')?selectedEncodedValues:selectedEncodedValues[0]);return}
if(defaultValue===''||defaultValue==null){return}
if(jQuery.isArray(defaultValue)){var encodedValues=[];for(var i=0;i<defaultValue.length;i++){if(typeof defaultValue[i]==='object'){encodedValues.push(wdtEncodeFilterOptionValue(defaultValue[i].value))}else if(defaultValue[i]){encodedValues.push(wdtEncodeFilterOptionValue(defaultValue[i]))}}
if(encodedValues.length){$select.val(encodedValues)}
return}
if(typeof defaultValue==='object'){defaultValue=defaultValue.value!=null?defaultValue.value:defaultValue.value}
if(defaultValue!==''&&defaultValue!=null){$select.val(wdtEncodeFilterOptionValue(defaultValue))}}
function wdtApplySelectFilterDefaultToPicker($select,defaultValue,selectedOptionValues){wdtSyncSelectFilterDefaultValue($select,defaultValue,selectedOptionValues);if(!$select.data('selectpicker')){return}
var currentValue=$select.val();if(currentValue==null||currentValue===''||(jQuery.isArray(currentValue)&&!currentValue.length)){return}
$select.selectpicker('val',currentValue);$select.selectpicker('render')}
function wdtCreateInput(oTable,aoColumn,columnIndex,sColumnLabel,th,serverSide){var bIsNumber=aoColumn.type==='number';var sCSSClass=aoColumn.type==='number'?'number_filter':'text_filter';sColumnLabel=sColumnLabel.replace(/(^\s*)|(\s*$)/g,"");sColumnLabel=sColumnLabel.replace(/<\/?span>/g,"");var placeholder=aoColumn.filterLabel?aoColumn.filterLabel:sColumnLabel;var input=jQuery('<input type="'+aoColumn.type+'" class="form-control wdt-filter-control '+sCSSClass+'" placeholder="'+_.escape(placeholder)+'" aria-label="'+wpdatatables_filter_strings.filterInputString_columnfilter+_.escape(placeholder)+'"/>');th.html(input);if(bIsNumber)
...
if(toDefaultValue){jQuery(to).val(toDefaultValue);if(tableDescription.table_wcag){jQuery(to).parent().parent().attr('data-value-from',toDefaultValue)}
if(!serverSide){jQuery(document).ready(function(){jQuery(to).trigger('blur')})}}}
function wdtCreateSelectbox(oTable,aoColumn,columnIndex,sColumnLabel,th,serverSide){var tableId=oTable.attr('id'),selected,selectedFilterValues=[];var tableDescription=JSON.parse(jQuery('#'+oTable.data('described-by')).val());if(aoColumn.values===null)
...
var selectTitle=aoColumn.filterLabel?_.escape(aoColumn.filterLabel):wpdatatables_filter_strings.nothingSelected_columnfilter;var select=jQuery('<select class="wdt-select-filter wdt-filter-control selectpicker" data-index="'+columnIndex+'" data-live-search="true" data-live-search-placeholder="'+wpdatatables_filter_strings.search_columnfilter+'">').attr('title',selectTitle);if(aoColumn.possibleValuesAjax!==-1){if(aoColumn.defaultValue){if(typeof aoColumn.defaultValue==='object'){select.append(buildSafeFilterOption(aoColumn.defaultValue.value,aoColumn.defaultValue.text,!0,!0));selectedFilterValues.push(wdtEncodeFilterOptionValue(aoColumn.defaultValue.value));oTable.api().column(columnIndex).search(aoColumn.defaultValue.value)}else{if(tableDescription.advanced_filter_option)aoColumn.defaultValue=wdtreplaceHtmlEntitiesForSpecialCharaters(aoColumn.defaultValue);select.append(buildSafeFilterOption(aoColumn.defaultValue,aoColumn.defaultValue,!0,!0));selectedFilterValues.push(wdtEncodeFilterOptionValue(aoColumn.defaultValue));oTable.api().column(columnIndex).search(aoColumn.defaultValue)}}}else{var iLen=aoColumn.values?aoColumn.values.length:0;for(var j=0;j<iLen;j++){selected='';if(aoColumn.defaultValue!==''&&aoColumn.defaultValue!=null){if(typeof aoColumn.defaultValue!=='object'&&tableDescription.advanced_filter_option){aoColumn.defaultValue=wdtreplaceHtmlEntitiesForSpecialCharaters(aoColumn.defaultValue)}
if(wdtFilterDefaultMatchesOptionValue(aoColumn.values[j].value,aoColumn.defaultValue)){selected='selected="selected" '}}
select.append(buildSafeFilterOption(aoColumn.values[j].value,aoColumn.values[j].label,!!selected,!0));if(selected){selectedFilterValues.push(wdtEncodeFilterOptionValue(aoColumn.values[j].value));oTable.api().column(columnIndex).search(aoColumn.values[j].value)}}}
wdtSyncSelectFilterDefaultValue(select,aoColumn.defaultValue,selectedFilterValues);th.html(select);th.wrapInner('<span class="filter_column filter_select" data-filter_type="selectbox" data-index="'+columnIndex+'"/>');select.on('change.selectChange',function(){if(tableDescription.advanced_filter_option){selectboxSearchSpecialCharacters(jQuery(this).val())}else{selectboxSearch.call(jQuery(this))}});if(aoColumn.possibleValuesAjax!==-1){select.on('show.bs.select',function(e){select.closest('.dropdown-menu').find('.optgroup-1').remove();select.closest('.filter_column').find('.bs-searchbox .form-control').val('').trigger('keyup');showSelectMultiSelectboxForFixedHeaderAndColumns(tableDescription,oTable,select)}).on('shown.bs.select',function(e){var bsContainer=jQuery('.bs-container').filter(function(){return jQuery(this).find('select').is(select)});if(bsContainer.length>0){bsContainer.css('width','auto');bsContainer.css('min-width','200px');bsContainer.find('.dropdown-menu').css('min-width','200px')}
...
select.selectpicker('refresh').ajaxSelectPicker({ajax:{url:wdt_ajax_object.ajaxurl,method:'POST',data:{wdtNonce:jQuery('#wdtNonceFrontendServerSide_'+oTable.data('wpdatatable_id')).val()||jQuery('#wdtFrontendelementorNonce_'+oTable.data('wpdatatable_id')).val(),action:'wpdatatables_get_column_possible_values',tableId:oTable.data('wpdatatable_id'),originalHeader:aoColumn.origHeader}},cache:!1,preprocessData:function(data){if(aoColumn.possibleValuesAddEmpty===!0){data.unshift({value:'possibleValuesAddEmpty',text:' '})}
return data},preserveSelected:!0,emptyRequest:!0,preserveSelectedPosition:'before',locale:{emptyTitle:wpdatatables_filter_strings.nothingSelected_columnfilter,statusSearching:wpdatatables_filter_strings.sLoadingRecords_columnfilter,currentlySelected:wpdatatables_filter_strings.currentlySelected_columnfilter,errorText:wpdatatables_filter_strings.errorText_columnfilter,searchPlaceholder:wpdatatables_filter_strings.search_columnfilter,statusInitialized:wpdatatables_filter_strings.statusInitialized_columnfilter,statusNoResults:wpdatatables_filter_strings.statusNoResults_columnfilter,statusTooShort:wpdatatables_filter_strings.statusTooShort_columnfilter}});if(aoColumn.defaultValue&&(!serverSide||tableDescription.advanced_filter_option)){select.trigger('change').data('AjaxBootstrapSelect').list.cache={}}
wdtApplySelectFilterDefaultToPicker(select,aoColumn.defaultValue,selectedFilterValues);if(aoColumn.searchInSelectBox!==1){jQuery(th).find('.bs-searchbox').hide()}else{jQuery(th).find('.bs-searchbox').show()}
wdtAddAttributesForWCAGSelectBox(select,'wdt-select-filter',tableDescription)}else{select.selectpicker('refresh');wdtApplySelectFilterDefaultToPicker(select,aoColumn.defaultValue,selectedFilterValues);if(aoColumn.searchInSelectBox!==1){jQuery(th).find('.bs-searchbox').hide()}else{jQuery(th).find('.bs-searchbox').show()}
...
if(aoColumn.defaultValue&&(!serverSide||tableDescription.advanced_filter_option)){oTable.fnFilter(aoColumn.defaultValue,columnIndex);wdtApplySelectFilterDefaultToPicker(select,aoColumn.defaultValue,selectedFilterValues)}}
...
if(typeof wpDataTables[tableId].drawTable==='undefined'||wpDataTables[tableId].drawTable===!0){oTable.api().draw()}
fnOnFiltered()}}
function selectboxSearchSpecialCharacters(value){if(value!==null){var search='';var cellValue=serverSide?value:jQuery.fn.dataTable.util.escapeRegex(value);if(aoColumn.possibleValuesAjax==-1)cellValue=decodeURIComponent(cellValue);if(serverSide&&aoColumn.possibleValuesAjax!==-1){cellValue=encodeURIComponent(cellValue)}
if(cellValue==='possibleValuesAddEmpty'&&!serverSide){oTable.api().column(columnIndex).search('^$',!0,!1)}else{if(aoColumn.exactFiltering){cellValue=serverSide?cellValue:cellValue.replace(/%/g,'%25');search=serverSide?decodeURIComponent(cellValue):'^'+decodeURIComponent(cellValue)+'$';oTable.api().column(columnIndex).search(cellValue?search:'',!0,!1)}else{cellValue=serverSide?cellValue:cellValue.replace(/%/g,'%25');oTable.api().column(columnIndex).search(decodeURIComponent(cellValue),!0,!1)}}
if(typeof wpDataTables[tableId].drawTable==='undefined'||wpDataTables[tableId].drawTable===!0){oTable.api().draw()}
fnOnFiltered()}}}
function wdtCreateMultiSelectbox(oTable,aoColumn,columnIndex,sColumnLabel,th,serverSide){var tableId=oTable.attr('id'),selected,selectedFilterValues=[];var tableDescription=JSON.parse(jQuery('#'+oTable.data('described-by')).val());if(aoColumn.values===null)
...
var selectTitle=aoColumn.filterLabel?_.escape(aoColumn.filterLabel):wpdatatables_filter_strings.nothingSelected_columnfilter;var select='<select class="wdt-multiselect-filter wdt-filter-control selectpicker" title="'+selectTitle+'" data-index="'+columnIndex+'" multiple data-live-search="true" data-live-search-placeholder="'+wpdatatables_filter_strings.search_columnfilter+'">';if(aoColumn.possibleValuesAjax!==-1){if(aoColumn.defaultValue[0]){var search='';for(i=0;i<aoColumn.defaultValue.length;i++){if(typeof aoColumn.defaultValue[i]==='object'){select+=buildSafeFilterOptionHtml(aoColumn.defaultValue[i].value,aoColumn.defaultValue[i].text,!0);search+=buildSearchStringForMultiFilters(wdtNormalizeRawFilterValueForSearch(aoColumn.defaultValue[i].value),aoColumn.exactFiltering,!serverSide);oTable.api().column(columnIndex).search(search.substring(0,search.length-1))}else{if(tableDescription.advanced_filter_option)aoColumn.defaultValue[i]=wdtreplaceHtmlEntitiesForSpecialCharaters(aoColumn.defaultValue[i]);select+=buildSafeFilterOptionHtml(aoColumn.defaultValue[i],aoColumn.defaultValue[i],!0);search+=buildSearchStringForMultiFilters(wdtNormalizeRawFilterValueForSearch(aoColumn.defaultValue[i]),aoColumn.exactFiltering,!serverSide);oTable.api().column(columnIndex).search(search.substring(0,search.length-1))}}}}else{var iLen=aoColumn.values?aoColumn.values.length:0;var search='';for(var j=0;j<iLen;j++){if(typeof aoColumn.defaultValue[0]==='object'){jQuery.each(aoColumn.defaultValue,function(index,value){selected=aoColumn.values[j].value.toString()==value.value?'selected="selected" ':'';if(selected!=='')
return!1})}else{if(tableDescription.advanced_filter_option)aoColumn.defaultValue=aoColumn.defaultValue.map(wdtreplaceHtmlEntitiesForSpecialCharaters);selected=jQuery.inArray(aoColumn.values[j].value.toString(),aoColumn.defaultValue)!==-1?'selected="selected" ':''}
select+=buildSafeFilterOptionHtml(aoColumn.values[j].value,aoColumn.values[j].label,!!selected);if(selected){selectedFilterValues.push(wdtEncodeFilterOptionValue(aoColumn.values[j].value));search+=buildSearchStringForMultiFilters(wdtNormalizeRawFilterValueForSearch(aoColumn.values[j].value),aoColumn.exactFiltering,!serverSide);oTable.api().column(columnIndex).search(search.substring(0,search.length-1))}}}
select=jQuery(select+'</select>');wdtSyncSelectFilterDefaultValue(select,aoColumn.defaultValue,selectedFilterValues);th.html(select);th.wrapInner('<span class="filter_column filter_select" data-filter_type="multiselectbox" data-index="'+columnIndex+'" />');select.on('change.selectChange',function(e){multiSelectboxSearch.call(jQuery(this))});if(aoColumn.possibleValuesAjax!==-1){select.on('show.bs.select',function(e){select.closest('.filter_column').find('.bs-searchbox .form-control').val('').trigger('keyup');showSelectMultiSelectboxForFixedHeaderAndColumns(tableDescription,oTable,select)}).on('shown.bs.select',function(e){var bsContainer=jQuery('.bs-container').filter(function(){return jQuery(this).find('select').is(select)});if(bsContainer.length>0){bsContainer.css('width','auto');bsContainer.css('min-width','200px');bsContainer.find('.dropdown-menu').css('min-width','200px')}
var searchBox=select.parent().find('.bs-searchbox .form-control');if(searchBox.length===0){var btnId=select.parent().find('button.dropdown-toggle').attr('aria-owns');if(btnId){searchBox=jQuery('#'+btnId).parent().find('.bs-searchbox .form-control')}}
...
select.selectpicker('refresh').ajaxSelectPicker({ajax:{url:wdt_ajax_object.ajaxurl,method:'POST',data:{wdtNonce:jQuery('#wdtNonceFrontendServerSide_'+oTable.data('wpdatatable_id')).val()||jQuery('#wdtFrontendelementorNonce_'+oTable.data('wpdatatable_id')).val(),action:'wpdatatables_get_column_possible_values',tableId:oTable.data('wpdatatable_id'),originalHeader:aoColumn.origHeader}},cache:!1,preprocessData:function(data){return data},preserveSelected:!0,emptyRequest:!0,preserveSelectedPosition:'before',locale:{emptyTitle:wpdatatables_filter_strings.nothingSelected_columnfilter,statusSearching:wpdatatables_filter_strings.sLoadingRecords_columnfilter,currentlySelected:wpdatatables_filter_strings.currentlySelected_columnfilter,errorText:wpdatatables_filter_strings.errorText_columnfilter,searchPlaceholder:wpdatatables_filter_strings.search_columnfilter,statusInitialized:wpdatatables_filter_strings.statusInitialized_columnfilter,statusNoResults:wpdatatables_filter_strings.statusNoResults_columnfilter,statusTooShort:wpdatatables_filter_strings.statusTooShort_columnfilter}});if(aoColumn.defaultValue[0]&&(!serverSide||tableDescription.advanced_filter_option)){select.trigger('change').data('AjaxBootstrapSelect').list.cache={}}
...
select.on('show.bs.select',function(e){showSelectMultiSelectboxForFixedHeaderAndColumns(tableDescription,oTable,select)}).on('hide.bs.select',function(e){hideSelectMultiSelectboxForFixedHeaderAndColumns(select)});if((!tableDescription.groupingEnabled&&select.closest('th').index()!=tableDescription.groupingColumnIndex)||!tableDescription.filterInForm){select.closest('.wdtscroll').on('scroll',function(e){select.closest('.wdt-multiselect-filter.open').removeClass('open');hideSelectMultiSelectboxForFixedHeaderAndColumns(select);if(!tableDescription.dataTableParams.fixedHeader.header&&tableDescription.dataTableParams.fixedColumns){wpDataTables[tableDescription.tableId].fnSettings().oInstance.api().draw()}})}
wdtAddAttributesForWCAGSelectBox(select,'wdt-multiselect-filter',tableDescription);if(aoColumn.defaultValue[0]&&(!serverSide||tableDescription.advanced_filter_option)){var search='';if(aoColumn.andLogic){for(var i=0;i<aoColumn.defaultValue.length;i++){search+=buildAndSearchStringForMultiFilters(wdtNormalizeRawFilterValueForSearch(aoColumn.defaultValue[i]),aoColumn.exactFiltering,i,!serverSide)}
if(aoColumn.exactFiltering){search=search.slice(0,-2);search+='$'}
oTable.fnFilter(search,columnIndex,!0,!1)}else{for(var i=0;i<aoColumn.defaultValue.length;i++){search+=buildSearchStringForMultiFilters(wdtNormalizeRawFilterValueForSearch(aoColumn.defaultValue[i]),aoColumn.exactFiltering,!serverSide)}
...
if(aoColumn.andLogic&&!serverSide){jQuery.each(selectedOptions,function(index,value){search+=buildAndSearchStringForMultiFilters(value,aoColumn.exactFiltering,index,!0)});if(aoColumn.exactFiltering){search=search.slice(0,-2);search+='$'}
...
search+=buildSearchStringForMultiFilters(value,aoColumn.exactFiltering,!serverSide)});oTable.api().column(columnIndex).search(search.substring(0,search.length-1),!0,!1)}
if(typeof wpDataTables[tableId].drawTable==='undefined'||wpDataTables[tableId].drawTable===!0){oTable.api().draw()}
fnOnFiltered()}}
...
if(checked&&!useAndLogic){search+=buildSearchStringForMultiFilters(wdtNormalizeRawFilterValueForSearch(value),aoColumn.exactFiltering,!serverSide);oTable.api().column(columnIndex).search(search.substring(0,search.length-1))}else if(checked){search+=buildAndSearchStringForMultiFilters(wdtNormalizeRawFilterValueForSearch(value),aoColumn.exactFiltering,i++,!serverSide)}}
...
if(aoColumn.defaultValue[0]&&!serverSide&&!useAndLogic){var search='';for(var i=0;i<aoColumn.defaultValue.length;i++){search+=buildSearchStringForMultiFilters(wdtNormalizeRawFilterValueForSearch(aoColumn.defaultValue[i]),aoColumn.exactFiltering,!0)}
...
search+=useAndLogic?buildAndSearchStringForMultiFilters(value,aoColumn.exactFiltering,i,!serverSide):buildSearchStringForMultiFilters(value,aoColumn.exactFiltering,!serverSide);i++});if(useAndLogic){if(aoColumn.exactFiltering){search=search.slice(0,-2);search+='$'}
...
return result}
function buildSearchStringForMultiFilters(value,exactFiltering,escapeForRegex){var search='',or='|';var decodedValue=wdtDecodeFilterOptionValue(value);if(escapeForRegex){decodedValue=wdtEscapeFilterSearchRegex(decodedValue)}
if(exactFiltering){search='^'+decodedValue+'$'+or}else{search=decodedValue+or}
return search}
function buildAndSearchStringForMultiFilters(value,exactFiltering,index,escapeForRegex){var search='';var decodedValue=wdtDecodeFilterOptionValue(value);if(escapeForRegex){decodedValue=wdtEscapeFilterSearchRegex(decodedValue)}
if(exactFiltering){if(index===0){search='^'}
search+=decodedValue+', '}else{search=search+'(?=.*'+decodedValue+')'}
return search}
...
wpDataTables[tableId].api().columns().search('').draw();wpDataTables[tableId].api().search('').draw();wpDataTableSelecter.find('.wdt-filter-control').eq(0).change()}})}
...
function wdtAddAttributesForDateTime(selector,element,selectorPicker){jQuery(selector).parent().parent().addClass('wpdt_using_wcag_filter');if(element.target.classList.contains(selectorPicker)){jQuery(selector).parent().parent().attr('data-value-from',jQuery(selector).val())}else{jQuery(selector).parent().parent().attr('data-value-to',jQuery(selector).val())}}
function wdtreplaceHtmlEntitiesForSpecialCharaters(text){var wdtTextAreaheleper=document.createElement('textarea');wdtTextAreaheleper.innerHTML=text;return wdtTextAreaheleper.value};(function($){"use strict";$.fn.rowGrouping=function(options){function _fnOnGrouped(){}
...
moment.locale(wpdatatables_frontend_strings.wdt_locale_language);$(this).datetimepicker({format:wdtDateFormat,showClear:!0,keepOpen:!1,useCurrent:!1,locale:wpdatatables_frontend_strings.wdt_locale_language,}).off('dp.show').on('dp.show',function(){$(this).parent().find('div.bootstrap-datetimepicker-widget').addClass('wdt-datetimepicker-modal');if(!_.contains(['MM/Y','MMM Y','Y'],wdtDateFormat)){wdtAddDatePlaceholders($(this))}
...
$selectpickerBlock.find('select').attr('multiple','multiple');if(column.defaultValue){var $editSelect=$selectpickerBlock.find('select');if(column.editorInputType==='multi-selectbox'){var defaultValues=!Array.isArray(column.defaultValue)?column.defaultValue.split('|'):column.defaultValue;defaultValuesArr=[];$.each(defaultValues,function(index,value){if(value){var optionValue=(typeof value==='object'&&value!==null&&value.value!=null)?value.value:value;var optionText=(typeof value==='object'&&value!==null&&value.text!=null)?value.text:optionValue;defaultValuesArr.push(optionValue);$editSelect.append(jQuery('<option></option>').prop('selected',!0).attr('value',optionValue).text(optionText))}})}else{if(typeof column.defaultValue==='object'){$editSelect.append(jQuery('<option></option>').prop('selected',!0).attr('value',column.defaultValue.value).text(column.defaultValue.text))}else{$editSelect.append(jQuery('<option></option>').prop('selected',!0).attr('value',column.defaultValue).text(column.defaultValue))}}}
...
if(column.defaultValue){let columnDefaultValue=column.defaultValue;if($.inArray(column.editorInputType,['selectbox','multi-selectbox'])!==-1){if(typeof defaultValuesArr==='undefined'){if(Array.isArray(columnDefaultValue)){defaultValuesArr=$.map(columnDefaultValue,function(value){return(value&&typeof value==='object'&&value.value!=null)?value.value:value})}else if(typeof columnDefaultValue==='object'&&columnDefaultValue!==null){defaultValuesArr=columnDefaultValue.value}else{defaultValuesArr=column.editorInputType==='multi-selectbox'?columnDefaultValue.split('|'):column.defaultValue}}
...
dt.on('column-reorder.dt.dtfc column-visibility.dt.dtfc column-sizing.dt.dtfc responsive-display.dt.dtfc',function(e,ctx){that.update()}).on('draw.dt.dtfc',function(e,ctx){that.update(ctx===dt.settings()[0]?!1:!0)});dt.on('destroy.dtfc',function(){that.destroy()});this._positions();this._scroll()},_clone:function(item,force){var that=this;var dt=this.s.dt;var itemDom=this.dom[item];var itemElement=item==='header'?this.dom.thead:this.dom.tfoot;var tableNode=$(dt.table().node());var scrollBody=$(tableNode.parent());var scrollLeftUpdate=function(){var scrollLeft=scrollBody.scrollLeft();that.s.scrollLeft={footer:scrollLeft,header:scrollLeft};if(itemDom.floatingParent){itemDom.floatingParent.scrollLeft(that.s.scrollLeft.header)}};if(item==='footer'&&this._scrollEnabled()){return}
if(!force&&itemDom.floating){itemDom.floating.removeClass('fixedHeader-floating fixedHeader-locked')}else{var docScrollLeft=$(document).scrollLeft();var docScrollTop=$(document).scrollTop();var bodyScrollLeft=scrollBody.scrollLeft();if($(this.wpDatatableDescription.selector).find('thead tr th span.filter_select div.btn-group.open').length!=0){$(this.wpDatatableDescription.selector).find('thead tr th span.filter_select div.btn-group.open').removeClass('open')}
...
var prop=item==='header'?'top':'bottom';var val=this.c[item+'Offset']-(shuffle>0?shuffle:0);var floatingLeft=scrollEnabled?scrollOffset.left:position.left;itemDom.floating.addClass('fixedHeader-floating');itemDom.floatingParent.css(prop,val).css({'left':floatingLeft,'height':item==='header'?'max-content':position.tfootHeight,'z-index':4}).append(itemDom.floating);if(!(this.wpDatatableDescription.scrollable))
importantWidth(position.width);if(scrollLeftUpdate){scrollLeftUpdate()}
if(item==='footer'){itemDom.floating.css('top','')}}else if(mode==='below'){var scrollLeftUpdateBelow=this._clone(item,forceChange);var belowLeft=scrollEnabled?scrollBody.offset().left:position.left;itemDom.floating.addClass('fixedHeader-locked');itemDom.floatingParent.css({position:'absolute',top:0,left:belowLeft+'px'});if(!(this.wpDatatableDescription.scrollable))
importantWidth(position.width);if(scrollLeftUpdateBelow){scrollLeftUpdateBelow()}}else if(mode==='above'){var scrollLeftUpdateAbove=this._clone(item,forceChange);var aboveLeft=scrollEnabled?scrollBody.offset().left:position.left;itemDom.floating.addClass('fixedHeader-locked');itemDom.floatingParent.css({position:'absolute',top:position.tbodyTop,left:aboveLeft+'px'});if(!(this.wpDatatableDescription.scrollable))
importantWidth(position.width);if(scrollLeftUpdateAbove){scrollLeftUpdateAbove()}}
