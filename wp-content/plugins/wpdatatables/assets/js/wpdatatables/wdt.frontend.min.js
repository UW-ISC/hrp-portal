...
th.html('');var sFromId=oTable.attr("id")+'_range_from_'+columnIndex;var fromHTML='<input type="text" class="form-control wdt-filter-control date-time-range-filter wdt-datetimepicker wdt-datetimepicker-from" id="'+sFromId+'" rel="'+columnIndex+'" placeholder="'+wpdatatables_filter_strings.from+'" />';var from=jQuery(fromHTML);var sToId=oTable.attr("id")+'_range_to_'+columnIndex;var toHTML='<input type="text" class="form-control wdt-filter-control date-time-range-filter wdt-datetimepicker wdt-datetimepicker-to" id="'+sToId+'" rel="'+columnIndex+'" placeholder="'+wpdatatables_filter_strings.to+'" />';var to=jQuery(toHTML);th.append(from).append(to);th.wrapInner('<span class="filter_column wdt-filter-date-time-range" data-filter_type="datetime range" data-index="'+columnIndex+'"/>');customSearchIndexes.push(columnIndex);oTable.dataTableExt.afnFiltering.push(function(oSettings,aData,iDataIndex){if(oTable.attr("id")!=oSettings.sTableId)
...
th.html('');var sFromId=oTable.attr("id")+'_range_from_'+columnIndex;var fromHTML='<input type="text" class="form-control wdt-filter-control time-range-filter wdt-timepicker wdt-timepicker-from" id="'+sFromId+'" rel="'+columnIndex+'" placeholder="'+wpdatatables_filter_strings.from+'" />';var from=jQuery(fromHTML);var sToId=oTable.attr("id")+'_range_to_'+columnIndex;var toHTML='<input type="text" class="form-control wdt-filter-control time-range-filter wdt-timepicker wdt-timepicker-to" id="'+sToId+'" rel="'+columnIndex+'" placeholder="'+wpdatatables_filter_strings.to+'" />';var to=jQuery(toHTML);th.append(from).append(to);th.wrapInner('<span class="filter_column filter_date_range" data-filter_type="time range" data-index="'+columnIndex+'"/>');customSearchIndexes.push(columnIndex);oTable.dataTableExt.afnFiltering.push(function(oSettings,aData,iDataIndex){if(oTable.attr("id")!=oSettings.sTableId)
...
return data},preserveSelected:!0,emptyRequest:!0,preserveSelectedPosition:'before',locale:{emptyTitle:wpdatatables_inline_strings.nothingSelected,statusSearching:wpdatatables_inline_strings.sLoadingRecords,currentlySelected:wpdatatables_inline_strings.currentlySelected,errorText:wpdatatables_inline_strings.errorText,searchPlaceholder:wpdatatables_inline_strings.search,statusInitialized:wpdatatables_inline_strings.statusInitialized,statusNoResults:wpdatatables_inline_strings.statusNoResults,statusTooShort:wpdatatables_inline_strings.statusTooShort}});$(obj.params.editSelector).on('show.bs.select',function(e){setTimeout(function(){$(obj.params.editSelector).trigger('change').data('AjaxBootstrapSelect').list.cache={};$(obj.params.editSelector).closest('.bootstrap-select').find('.bs-searchbox .form-control').val('').trigger('keyup')},500);if(obj.params.value!==null){if(obj.params.inputType==='multi-selectbox'){var selectedValues=!Array.isArray(obj.params.value)?obj.params.value.split(', '):obj.params.value;$.each(selectedValues,function(index,value){if(value){$(obj.params.editSelector).append('<option selected value="'+value+'">'+value+'</option>')}})}else{$(obj.params.editSelector).append('<option selected value="'+obj.params.value+'">'+obj.params.value+'</option>')}
...
if(!isHandsonTable){var wpTableDescription=$(this).closest('.wdt-constructor-default-value').length||$(this).closest('.wpDataTableFilterSection').length||$(this).closest('.modal-body').length||$(this).closest('.wdt-editing-enabled-block').length||$(this).closest('.wdt-filter-default-value-from-block').length||$(this).closest('.wdt-filter-default-value-to-block').length?'':JSON.parse(jQuery('#'+$(this).parents('table').data().describedBy).val());var filterElem='none';if($(this).closest('.wpDataTableFilterSection').length==0&&$(this).closest('.modal-body').length==0&&$(this).closest('.wdt-constructor-default-value').length==0&&$(this).closest('.wdt-editing-enabled-block').length==0&&$(this).closest('.wdt-filter-default-value-from-block').length==0&&$(this).closest('.wdt-filter-default-value-to-block').length==0){if(wpTableDescription.dataTableParams.fixedColumns||wpTableDescription.dataTableParams.fixedHeader.header){filterElem=wpTableDescription.renderFilter==="header"?'th':'td'}}}
...
for(var i=0;i<tableDescription.transform_value_columns.length;i++){var params={};var column=oSettings.oInstance.api().column(tableDescription.transform_value_columns[i]+':name',{search:'applied'});var transformValueRules={};transformValueRules[0]=oSettings.aoColumns[column.index()].transformValueRules;for(var k=0;k<oSettings.aoColumns.length;k++){var col=oSettings.oInstance.api().column(oSettings.aoColumns[k].name+':name',{search:'applied'});var nodes=column.nodes();column.nodes().to$().each(function(){if(transformValueRules[0].includes('{'+oSettings.aoColumns[k].name+'.value}')){for(var m=0;m<col.data().length;m++){if(m!=0){if(transformValueRules[m+1]!=null){wdtTransformValue(transformValueRules[m+1].replaceAll(col.data()[m-1],col.data()[m]===null?'':col.data()[m]),params,$(this),m,oSettings.sTableId);transformValueRules[m+1]=transformValueRules[m+1].replaceAll('{'+oSettings.aoColumns[k].name+'.value}',col.data()[m]===null?'':col.data()[m])}else{wdtTransformValue(transformValueRules[m].replaceAll(col.data()[m-1],col.data()[m]===null?'':col.data()[m]),params,$(this),m,oSettings.sTableId);transformValueRules[m+1]=transformValueRules[m].replaceAll(col.data()[m-1],col.data()[m]===null?'':col.data()[m])}}else{if(transformValueRules[m+1]!=null){wdtTransformValue(transformValueRules[m+1].replaceAll('{'+oSettings.aoColumns[k].name+'.value}',col.data()[m]===null?'':col.data()[m]),params,$(this),m,oSettings.sTableId);transformValueRules[m+1]=transformValueRules[m+1].replaceAll('{'+oSettings.aoColumns[k].name+'.value}',col.data()[m]===null?'':col.data()[m])}else{wdtTransformValue(transformValueRules[m].replaceAll('{'+oSettings.aoColumns[k].name+'.value}',col.data()[m]===null?'':col.data()[m]),params,$(this),m,oSettings.sTableId);transformValueRules[m+1]=transformValueRules[m].replaceAll('{'+oSettings.aoColumns[k].name+'.value}',col.data()[m]===null?'':col.data()[m])}}}}})}}}});if(!tableDescription.serverSide){wpDataTables[tableDescription.tableId].fnDraw()}}
