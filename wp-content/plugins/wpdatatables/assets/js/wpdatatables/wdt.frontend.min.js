...
select+=buildSafeFilterOptionHtml(aoColumn.values[j].value,aoColumn.values[j].label,!!selected);if(selected){selectedFilterValues.push(wdtEncodeFilterOptionValue(aoColumn.values[j].value));search+=buildSearchStringForMultiFilters(aoColumn.values[j].value,aoColumn.exactFiltering);oTable.api().column(columnIndex).search(search.substring(0,search.length-1))}}}
...
if(tableDescription.advanced_filter_option){if(aoColumn.defaultValue[0]===''){value=serverSide?value:jQuery.fn.dataTable.util.escapeRegex(value)}}
...
return result}
function buildSearchStringForMultiFilters(value,exactFiltering){var search='',or='|';var encodedValue=encodeURIComponent(value.toString().replace(/\+/g,'\\+'));if(exactFiltering){search='^'+encodedValue+'$'+or}else{search=encodedValue+or}
