(function(Handsontable,$){function wdtEscapeHtmlPlain(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;')}
function wdtIsWpdtSafeWebUrl(url){url=String(url==null?'':url).trim();if(!url){return!1}
if(url.indexOf('//')===0){return!1}
if(url.charAt(0)==='/'&&url.charAt(1)!=='/'){return!0}
var lower=url.toLowerCase();if(lower.indexOf('http://')!==0&&lower.indexOf('https://')!==0){return!1}
try{var u=new URL(url);var h=(u.hostname||'').toLowerCase();if(!h){return!1}
if(h==='localhost'){return!0}
if(h.charAt(0)==='['&&h.charAt(h.length-1)===']'){return!0}
if(h.indexOf('.')!==-1){return!0}
if(/^\d{1,3}(\.\d{1,3}){3}$/.test(h)){return!0}
return!1}catch(e){return!1}}
function wdtSanitizeExcelHref(url){url=String(url==null?'':url).trim();if(!url){return''}
if(/["'<>]/.test(url)||/[\u0000-\u001F\u007F]/.test(url)){return''}
return wdtIsWpdtSafeWebUrl(url)?url:''}
function wdtSanitizeExcelMailto(addr){addr=String(addr==null?'':addr).trim();if(!addr||/["'<>]/.test(addr)||/[\u0000-\u001F\u007F]/.test(addr)){return''}
return addr}
function wdtExcelRichCellHrefMustStrip(val){val=String(val==null?'':val).trim();if(!val){return!0}
if(val.indexOf('//')===0){return!0}
var m=val.match(/^([a-z][a-z0-9+.-]*):/i);if(!m){return!1}
var s=m[1].toLowerCase();if(s==='javascript'||s==='vbscript'||s==='data'||s==='file'){return!0}
return!1}
function wdtSanitizeUrlAttrForExcelCell(val,attrName){val=String(val==null?'':val).trim();attrName=String(attrName==null?'':attrName).toLowerCase();if(!val){return''}
if(/^\s*(javascript|vbscript)\s*:/i.test(val)||/^\s*data\s*:\s*text\/html/i.test(val)){return''}
if(/["'<>]/.test(val)||/[\u0000-\u001F\u007F]/.test(val)){return''}
if(attrName==='href'){return wdtExcelRichCellHrefMustStrip(val)?'':val}
if(attrName==='src'||attrName==='poster'||attrName==='formaction'){return wdtIsWpdtSafeWebUrl(val)?val:''}
return wdtIsWpdtSafeWebUrl(val)?val:''}
function wdtSanitizeExcelRichCellHtml(html){if(typeof html!=='string'||html.indexOf('<')===-1){return html}
if(typeof DOMParser==='undefined'){return wdtEscapeHtmlPlain(html)}
try{var doc=new DOMParser().parseFromString('<div data-wdt-excel-san="1">'+html+'</div>','text/html');var wrapper=doc.body&&doc.body.firstElementChild;if(!wrapper){return wdtEscapeHtmlPlain(html)}
var urlAttrs=['href','src','poster','formaction'];var nodes=wrapper.getElementsByTagName('*');var i,j,el,nm,v,safe;for(i=nodes.length-1;i>=0;i--){el=nodes[i];for(j=el.attributes.length-1;j>=0;j--){nm=el.attributes[j].name;if(/^on/i.test(nm)){el.removeAttribute(nm)}}
if(el.hasAttribute('srcdoc')){el.removeAttribute('srcdoc')}}
nodes=wrapper.getElementsByTagName('*');for(i=0;i<nodes.length;i++){el=nodes[i];for(j=0;j<urlAttrs.length;j++){nm=urlAttrs[j];if(!el.hasAttribute(nm)){continue}
v=el.getAttribute(nm);safe=wdtSanitizeUrlAttrForExcelCell(v,nm);if(safe===''){el.removeAttribute(nm)}else{el.setAttribute(nm,safe)}}}
var anchors=wrapper.querySelectorAll('a');for(i=0;i<anchors.length;i++){el=anchors[i];if(!String(el.getAttribute('href')||'').trim()){while(el.firstChild){el.parentNode.insertBefore(el.firstChild,el)}
if(el.parentNode){el.parentNode.removeChild(el)}}}
return wrapper.innerHTML}catch(err){return wdtEscapeHtmlPlain(html)}}
var baseBeginEditing=Handsontable.editors.BaseEditor.prototype.beginEditing;Handsontable.editors.BaseEditor.prototype.beginEditing=function(initialValue,event){var settings=this.instance.getSettings();if(settings.wpDataTablesExcelPlugin){if(typeof initialValue=='undefined'&&!settings.readOnly){var idColProp=settings.idColumnKey;var idColVal=this.instance.getDataAtRowProp(this.row,idColProp);var defaultValue=this.cellProperties.defaultValue;if(!idColVal&&defaultValue){initialValue=defaultValue}}}
...
var link_parts=value.split('||');var link=wdtSanitizeExcelHref(link_parts[0]);if(link.length==0){$(td).empty();td.appendChild(document.createTextNode(String(link_parts[1]!=null?link_parts[1]:link_parts[0])));return td}
var content=(link_parts.length>1)?link_parts[1]:link;var table_settings=instance.getSettings();var class_name='wdt_link';var titleText='';if(!table_settings.readOnly){class_name+='_editable';titleText='ctrl+click to open hyperlink:'+link}
var $a=$('<a/>',{'class':class_name,target:'_blank',rel:'noopener noreferrer',href:link});if(titleText){$a.attr('title',titleText)}
$a.text(content);$(td).empty().append($a)}
...
var email_parts=value.split('||');var email=wdtSanitizeExcelMailto(email_parts[0]);if(email.length==0){$(td).empty();td.appendChild(document.createTextNode(String(email_parts[1]!=null?email_parts[1]:email_parts[0])));return td}
var content=(email_parts.length>1)?email_parts[1]:email;var table_settings=instance.getSettings();var class_name='wdt_email';var titleText='';if(!table_settings.readOnly){class_name+='_editable';titleText='ctrl+click to send email to:'+email}
var $a=$('<a/>',{'class':class_name,href:'mailto:'+email});if(titleText){$a.attr('title',titleText)}
$a.text(content);$(td).empty().append($a)}
...
var image_parts=value.split('||');var raw_thumb=image_parts[0];if(raw_thumb.length==0){return td}
var imageUri=new URI(raw_thumb);var imageQuery=imageUri.query(!0);var img_width=(imageUri.hasQuery('img_width'))?imageQuery.img_width:0;var img_height=(imageUri.hasQuery('img_height'))?imageQuery.img_height:0;var thumb_src=wdtSanitizeExcelHref(imageUri.toString());if(!thumb_src){$(td).empty();return td}
var img_style='';if(img_width>0){img_style+='width:'+img_width+'px; '}
if(img_height>0){img_style+='height:'+img_height+'px;'}
if(img_style.length>0){img_style='style="'+img_style+'"'}
var html='';if(image_parts.length>1&&image_parts[1]!=''){var tableSettings=instance.getSettings();var class_name='wdt_link';var title_attr='';var fullSizeImgUri=new URI(image_parts[1]);fullSizeImgUri.removeQuery(['img_width','img_height']);var full_size_image=wdtSanitizeExcelHref(fullSizeImgUri.toString());if(!full_size_image){html='<img class="wpdt-thumb" src="'+thumb_src+'" '+img_style+' />'}else{if(!tableSettings.readOnly){class_name+='_editable';title_attr='title="ctrl+click to open hyperlink:'+full_size_image+'"'}
var image_html='<img class="wpdt-thumb" src="'+thumb_src+'" '+img_style+' />';html='<a class="'+class_name+'" href="'+full_size_image+'" target="_blank" rel="noopener noreferrer" '+title_attr+'>'+image_html+'</a>'}}else{html='<img class="wpdt-thumb" src="'+thumb_src+'" '+img_style+' />'}
td.innerHTML=html}
function wdtTextRenderer(instance,td,row,col,prop,value,cellProperties){var escaped=Handsontable.helper.stringify(value);escaped=wdtStripTags(escaped,'<br/><br><b><strong><h1><h2><h3><a><i><em><ol><ul><li><img><blockquote><div><hr><p><span><select><option><sup><sub>');escaped=wdtSanitizeExcelRichCellHtml(escaped);td.innerHTML=escaped;Handsontable.SearchCellDecorator.apply(this,arguments);return td}
