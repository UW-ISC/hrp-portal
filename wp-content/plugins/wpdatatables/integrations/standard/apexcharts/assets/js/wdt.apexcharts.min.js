...
this.options.yaxis[i].opposite=chartConfig.yaxis[i].opposite;this.options.yaxis[i].seriesName=this.options.yaxis[i].opposite?chartConfig.series[i].name:chartConfig.series[0].name;if(!this.options.yaxis[i].title.text)this.options.yaxis[i].title.text=this.options.series[i].label}
...
this.setDecimalsInFloat()},setSeriesAndAxis:function(chartConfig){this.setBackground(chartConfig.chart.background);if(this.options.xaxis.title)
this.options.xaxis.title.position="bottom";if(['line','area','column'].includes(chartConfig.chart.type)){var j=0;for(var i in chartConfig.series){this.options.colors[j]=chartConfig.series[i].color?chartConfig.series[i].color:this.options.colors[j];this.options.fill.image[j]=chartConfig.series[i].chart_image?chartConfig.series[i].chart_image:'';if(chartConfig.series[i].type)
this.options.series[j].type=chartConfig.series[i].type;this.options.yaxis[j]={title:{text:chartConfig.series[i].name,},label:chartConfig.series[i].label,show:j===0||chartConfig.series[i].yAxis,crosshairs:{show:chartConfig.yaxis[0].crosshairs.show,},tooltip:{enabled:chartConfig.yaxis[0].tooltip.enabled},seriesName:chartConfig.series[i].yAxis==1?this.options.series[j].name:this.options.series[0].name,min:chartConfig.yaxis[0].min?parseFloat(chartConfig.yaxis[0].min):undefined,max:chartConfig.yaxis[0].max?parseFloat(chartConfig.yaxis[0].max):undefined,tickAmount:chartConfig.yaxis[0].tickAmount!='0'&&chartConfig.yaxis[0].tickAmount!=undefined?parseInt(chartConfig.yaxis[0].tickAmount):undefined,opposite:chartConfig.series[i].yAxis==1,reversed:chartConfig.yaxis[i].reversed,};j++;this.options.series[i].name=this.options.series[i].label}}},setAxisTitles:function(){if(!this.options.xaxis.title.text)
this.options.xaxis.title.text=this.options.series[0].label;if(Array.isArray(this.options.yaxis)&&!this.options.yaxis[0].title.text)
this.options.yaxis[0].title.text=this.options.series[0].label},setStrokeWidth:function(width){if(width)this.options.stroke.width=parseInt(width);},isColorValid:function(color){var e=document.getElementById('divValidColor');if(!e){e=document.createElement('div');e.id='divValidColor'}
e.style.borderColor='';e.style.borderColor=color;var tmpcolor=e.style.borderColor;return tmpcolor.length!=0},assesGradientFillType:function(imageSource){return imageSource!=''||(Array.isArray(imageSource)&&imageSource.length)},setType:function(type){switch(type){case 'apexcharts_spline_chart':this.options.chart.type='line';this.options.stroke.curve='smooth';this.options.plotOptions.bar.horizontal=!1;break;case 'apexcharts_stepline_chart':this.options.chart.type='line';this.options.stroke.curve='stepline';this.options.plotOptions.bar.horizontal=!1;break;case 'apexcharts_area_chart':this.options.chart.type='area';this.options.plotOptions.bar.horizontal=!1;break;case 'apexcharts_spline_area_chart':this.options.chart.type='area';this.options.stroke.curve='smooth';this.options.plotOptions.bar.horizontal=!1;break;case 'apexcharts_basic_area_chart':this.options.chart.type='area';this.options.stroke.curve='straight';this.options.plotOptions.bar.horizontal=!1;break;case 'apexcharts_stepline_area_chart':this.options.chart.type='area';this.options.stroke.curve='stepline';this.options.plotOptions.bar.horizontal=!1;break;case 'apexcharts_grouped_bar_chart':this.options.chart.type='bar';this.options.plotOptions.bar.horizontal=!0;break;case 'apexcharts_stacked_bar_chart':this.options.chart.type='bar';this.options.chart.stacked=!0;this.options.plotOptions.bar.horizontal=!0;break;case 'apexcharts_100_stacked_bar_chart':this.options.chart.type='bar';this.options.chart.stacked=!0;this.options.chart.stackType='100%'
...
this.setStrokeWidth(this.options.stroke.width);this.setDecimalsInFloat();this.chart=new ApexCharts(document.querySelector(container),this.options);var chartID=this.container.replace(/.*_(\d+)/,'$1');if(jQuery(this.container).parent().find('.wdt-wrapper-chart-loader').length!=0){jQuery(this.container).parent().find('.wdt-wrapper-chart-loader').each(function(){if(jQuery(this).attr('data-id')===chartID){jQuery(this).hide()}})}
this.chart.render()},refresh:function(){this.chart.render()},setConnectedWPDataTable:function(wpDataTable){this.connectedWPDataTable=wpDataTable},setNumberFormat:function(numberFormat){this.numberFormat=numberFormat},getNumberFormat:function(){return this.numberFormat},setDecimalPlaces:function(decimalPlaces){this.decimalPlaces=decimalPlaces},getDecimalPlaces:function(){return this.decimalPlaces},setDecimalsInFloat:function(){var decimals=this.getDecimalPlaces()?this.getDecimalPlaces():2;if(Array.isArray(this.options.yaxis)){for(var i in this.options.yaxis){this.options.yaxis[i].decimalsInFloat=decimals}}else{this.options.yaxis.decimalsInFloat=decimals}},setGrouping:function(group_chart){this.group_chart=group_chart},setLoader:function(loader){this.loader=loader},setColumnIndexes:function(columnIndexes){this.columnIndexes=columnIndexes},formUrlFromSource:function(sourceUrl){if(sourceUrl&&sourceUrl!==''){if(sourceUrl.startsWith('url('))return sourceUrl;return'url('+sourceUrl+') no-repeat center/cover'}else return'#FFFFFF'},setChartConfig:function(chartConfig){if(chartConfig.responsive_width!==1){this.setWidth(chartConfig.width)}else this.setWidth(0);this.setResponsiveWidth(chartConfig.responsive_width);this.setHeight(chartConfig.height);this.options.chart.animations.enabled=chartConfig.enable_animation===1;if(this.options.chart.type==='pie'){if(chartConfig.show_data_labels===1){this.options.dataLabels.enabled=!0;this.options.plotOptions.pie={dataLabels:{minAngleToShowLabel:0}}}else{this.options.dataLabels.enabled=!1}}else if(this.options.chart.type==='radialBar'){this.options.plotOptions.radialBar.dataLabels.show=chartConfig.show_data_labels===1}else{this.options.dataLabels.enabled=chartConfig.show_data_labels===1}
...
if(chartConfig.line_background_image!=''){this.options.fill.type='image';this.options.fill.image.src=chartConfig.line_background_image}else{this.options.fill.type=['apexcharts_pie_with_gradient_chart','apexcharts_donut_with_gradient_chart'].includes(chartConfig.type)?'gradient':'solid';this.options.fill.image.src=[]}
...
obj.options.xaxis.categories=outputCategories;seriesDataEntry=outputData;if(isSingleSeriesType){obj.options.series=seriesDataEntry}else{obj.options.series[seriesIndex].data=seriesDataEntry}}else{if(isSingleSeriesType){obj.options.series=[]}else{obj.options.series[j-1].data=[]}
