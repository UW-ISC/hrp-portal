...
//            add_filter( 'wpdatatables_filter_cell_output', array( self::$calledClassName, 'parseWPDatatablesFields' ), 20000 );
    ...
    public static function addTooltipScriptArgs( $tooltipArgs ) {
        ...
        $tooltipArgs[ 'close_symbol' ] = get_option( 'cmtt_tooltipCloseSymbol', 'dashicons-no' );
        return $tooltipArgs;
    }
...
//		set_transient( $transientKey, $parserArrays, 60 * 60 * 24 * 30 );
    ...
    public static function addTermPageLinkToTooltip( $glossaryItemContent, $glossary_item ) {
        $tooltipOnClick          = get_option( 'cmtt_glossaryShowTooltipOnClick', '0' );
        $addLink                 = get_option( 'cmtt_glossaryAddTermPagelink' );
        $createGlossaryTermPages = (bool) get_option( 'cmtt_createGlossaryTermPages', TRUE );

        if ( $createGlossaryTermPages && ($addLink == 1 || $tooltipOnClick == 1 ) ) {
            $target              = get_option( 'cmtt_glossaryTermPageLinkTargetBlank', false ) ? 'target=&quot; _blank &quot;' : '';
...
//                str_replace( array( "\r\n", "\n", "\r" ), array( "", "", "" ), nl2br( $term->post_content ) ),
    ...
    public static function importGlossaryItem( $item, $override = TRUE ) {
        if ( !empty( $item ) && is_array( $item ) && count( $item ) >= 4 ) {
            /*
             * Empty title
             */
            if ( empty( $item[ 1 ] ) ) {
            ...
            if ( $update > 0 && !empty( $item[ 10 ] ) ) {
                self::uploadImage( $item[ 10 ], $update, '' );
            }
    ...
    public static function _cmtt_importGlossary( $file ) {
        ...
        $queryArgs = array( 'msg' => 'imported', 'itemstotal' => $allElements, 'itemsimported' => $importedElements, 'error' => $error );
    ...
    public static function cmtt_glossary_addBacklink( $content = '' ) {
        ...
        if ( $addBacklink ) {
            ...
            $backlinkBottom = (get_option( 'cmtt_glossary_addBackLinkBottom' ) == 1 && $mainPageId > 0) ? '<a href="' . $backlinkHref . '" class="cmtt-backlink cmtt-backlink-bottom">' . __( get_option( 'cmtt_glossary_backLinkBottomText' ), 'cm-tooltip-glossary' ) . '</a>' : '';
