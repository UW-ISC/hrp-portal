...
class CMTT_Related {
            ...
            foreach ( $glossaryIds as $glossaryId ) {
                if ( $glossaryId != $id ) {
                    ...
                    if ( !$result ) {
                         ...
                         */
                        update_option( 'cmtt_glossary_relatedArticlesIndexingInsertError', '1' );
                    }
                }
            }
        }
    ...
    public static function getParsingProblems() {
        $result = '';
        if ( get_option( 'cmtt_glossary_relatedArticlesIndexingStarted' ) ) {
            $result .= 'Error: Chunk processing timed-out. Please lower the chunk size and retry.';
        }
        if ( get_option( 'cmtt_glossary_relatedArticlesIndexingInsertError' ) ) {
    ...
    public static function crawlArticles( $restart = false ) {
         ...
         * Clear error
         */
        update_option( 'cmtt_glossary_relatedArticlesIndexingInsertError', '0' );
        update_option( 'cmtt_glossary_relatedArticlesIndexingStarted', '1' );
         ...
         * Indicate the finish
         */
        update_option( 'cmtt_glossary_relatedArticlesIndexingStarted', '0' );
    ...
    public static function getRelatedArticles( $glossaryId, $limit = 5, $type = 'all' ) {
        ...
        $sql     = $wpdb->prepare( "SELECT p.ID, p.post_title, p.post_excerpt, p.post_type FROM {$wpdb->prefix}" . self::TABLENAME . " g JOIN {$wpdb->posts} p ON g.articleId=p.ID AND g.glossaryId=%d " . $where . " ORDER BY " . $order . " LIMIT %d", $glossaryId, $limit );
