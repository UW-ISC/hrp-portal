...
class CMTT_Glossary_Plus {
    ...
    public static function init() {
        ...
        /* RTL Style */
        if ( get_option( 'cmtt_glossaryRTL' ) == 1 ) {
        ...
        add_filter( 'cmtt_3rdparty_tooltip_content', array( __CLASS__, 'cmtt_add_featured_image_to_tooltip_content' ), 10, 2 );

        /*
         * Custom  Index Link
         */
        // Add/Edit
        add_action( 'glossary-categories_add_form_fields', array( __CLASS__, 'taxonomyIndexLinkField' ), 10, 2 );
        add_action( 'glossary-categories_edit_form_fields', array( __CLASS__, 'taxonomyIndexLinkField' ), 10, 2 );
        // Save
        add_action( 'edited_glossary-categories', array( __CLASS__, 'saveIndexLinkField' ), 10, 2 );
        add_action( 'created_glossary-categories', array( __CLASS__, 'saveIndexLinkField' ), 10, 2 );
        // Use
    ...
    public static function taxonomyIndexLinkField( $tag ) {
        $link = (is_object( $tag )) ? get_term_meta( $tag->term_id, 'cmtt_category_index_link', true ) : '';
        ?>
        <tr class="form-field form-required term-name-wrap">
            <th scope="row"><label for="cmtt_category_index_link"><?php _e( 'Taxonomy Index Link', CMTT_SLUG_NAME ); ?></label></th>
            <td><input name="cmtt_category_index_link" id="cmtt_category_index_link" value="<?php echo esc_attr( $link ); ?>" type="text" size="40" />
                <p class="description"><?php _e( 'Enter the exact url of the page where the "Back to Business Index" button should link to.', CMTT_SLUG_NAME ); ?></p>
            </td>
        </tr>
        <?php
    }

    public static function saveIndexLinkField( $term_id, $tt_id ) {
        if ( isset( $_POST[ 'cmtt_category_index_link' ] ) ) {
            $link = $_POST[ 'cmtt_category_index_link' ];
     ...
     * @return type
     */
    public static function cmtt_add_featured_image_to_tooltip_content( $glossaryItemContent, $post ) {
        /* ML - get the location of the image from settings
         */
        $featuredImageLocationGeneric = get_option( 'cmtt_glossary_tooltip_featuredImageDisplay', 'above_content' );
        $featuredImageLocationMeta    = get_post_meta( $post->ID, '_cmtt_featured_image_position', true );

        $featuredImageLocation = !empty( $featuredImageLocationMeta ) ? $featuredImageLocationMeta : $featuredImageLocationGeneric;

        if ( $featuredImageLocation === 'no' ) {
        ...
        $thumbnail = get_the_post_thumbnail( $post->ID, 'full', array( 'style' => 'width: ' . $imageWidth . '; margin-top: 2%;' . $additionalStyle ) );
...
//		$terms = wp_get_post_terms( $glossary_item->ID, 'glossary-categories' );
    ...
    public static function outputCategories( $shortcodeAtts, $glossary_query ) {
        ...
        $glossaryCategoriesAllDisabled = get_option( 'cmtt_glossary_disableAllCats', '0' );
...
//			$glossaryIndex	 = $query->query( $args );
    ...
    public static function processGlossaryIndexShortcodeAtts( $baseAtts ) {
        ...
        if ( $glossaryCategoriesAllDisabled && 'all' === $baseAtts[ 'cat' ] ) {
             ...
             * Code allows to display only the relevant categories meaning that only the categories of the currently displayed items will be displayed.
             */
            $showOnlyRelevant = isset( $baseAtts[ 'only_relevant_cats' ] ) ? $baseAtts[ 'only_relevant_cats' ] : FALSE;
            if ( $showOnlyRelevant ) {
                $args      = isset( CMTT_Pro::$lastQueryDetails[ 'nopaging_args' ] ) ? CMTT_Pro::$lastQueryDetails[ 'nopaging_args' ] : array();
                $posts_ids = array();
            ...
            if ( get_option( 'cmtt_glossaryUseTemplate' ) && $post->post_type == "glossary" ) {
                ...
                if ( get_option( 'cmtt_glossaryPageTermTemplate' ) == '1' ) {
                    if ( file_exists( $theme_path . '/Tooltip/single-sidebar-glossary.php' ) && file_exists( $theme_path . '/Tooltip/sidebar-glossary.php' ) ) {
                        ...
                        $single = $glossary_path . 'theme/Tooltip/single-sidebar-glossary.php';
                        return $single;
                    }
                } else {
    ...
    public static function cmtt_custom_sidebar() {
        register_sidebar( array(
            ...
            'before_widget' => '<div class="topbar-widget-container">',
            'after_widget'  => '</div>',
    ...
    public static function addAbbreviation( $title = '', $id = null ) {
        if ( $id ) {
            ...
            if ( $glossaryItem && 'glossary' == $glossaryItem->post_type ) {
                ...
                if ( $abbreviation ) {
                    if ( get_option( 'cmtt_roundBracketsAbbr' ) == 'round' ) {
                        $title .= ' (' . $abbreviation . ')';
                    } else {
                        $title .= ' [' . $abbreviation . ']';
                    }
    ...
    public static function addRelatedTerms( $content = '' ) {
        ...
        if ( !in_array( $id, $added ) && is_singular() && $wp_query->is_main_query() && (get_option( 'cmtt_showRelatedTermsList' ) == 1) && !$disableRelatedTermsForPage && !$disableRelatedTermsOnTermPages ) {
            $added[]        = $id;
     ...
     * RTL
     */
    public function _rtl_support() {
        wp_add_inline_style( 'cmtooltip', '
			#tt {
    ...
    public static function _image_tiles_view( $id = null ) {
        ...
        if ( !empty( $thumbnail ) ) {
            $result[ 'thumbnail' ] = '<a class="cmtt-thumbnail" target="_blank" href="' . $permalink . '">' . $thumbnail . '</a>';
        } else {
            if ( get_option( 'cmtt_glossary_no_thumb' ) != '' ) {
                $src                   = wp_get_attachment_image_src( get_option( 'cmtt_glossary_no_thumb' ), array( 250, 250 ), false );
        ...
        $result[ 'liAdditionalClass' ] = $liAdditionalClass             = 'cmtt-has-thumbnail';
        return $result;
    }
    ...
    public static function _image_uploader( $name ) {
        wp_enqueue_media();
        $src      = wp_get_attachment_image_src( get_option( 'cmtt_glossary_no_thumb' ), array( 250, 250 ), false );
        (get_option( $name ) != '') ? $hasThumb = 'cmtt_hasThumb' : $hasThumb = '';
        ($src[ 0 ]) ? $image    = $src[ 0 ] : $image    = '';
        $src      = wp_get_attachment_image_src( get_option( 'cmtt_glossary_no_thumb' ), array( 250, 250 ), false );
        $q        = '';
        $q .= '
			<div id="glossary_no_thumb-preview" class="cmtt_Media_Image ' . $hasThumb . '" style="background-image:url(\'' . $image . '\')"></div>
			<input id="' . $name . '" class="' . $name . ' cmtt_Media_Storage" name="' . $name . '" type="hidden" value="' . get_option( $name ) . '" />
