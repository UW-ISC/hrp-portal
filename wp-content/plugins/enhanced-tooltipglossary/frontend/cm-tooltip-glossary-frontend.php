...
//		$normalizedTitle = str_replace( '&#039;', "’", preg_quote( htmlspecialchars( trim( $title ), ENT_QUOTES, 'UTF-8' ), '/' ) ); //changed in 3.3.8
    ...
    public static function cmtt_glossary_filterTooltipContent( $glossaryItemContent, $glossaryItemPermalink ) {
        ...
        if ( ($tooltipLengthLimit >= 30) && (strlen( $glossaryItemContent ) > $tooltipLengthLimit) ) {
            $text                = __( get_option( 'cmtt_glossaryTermDetailsLink', 'Term details' ) );
            $link                = get_option( 'cmtt_tooltipIsClickable' ) ? '<a class="glossaryTooltipMoreLink" href="' . $glossaryItemPermalink . '">' . $text . '</a>' : '';
            $glossaryItemContent = cminds_truncate( html_entity_decode( $glossaryItemContent ), $tooltipLengthLimit, '(...)' ) . ' <strong>' . $link . '</strong>';
        }
