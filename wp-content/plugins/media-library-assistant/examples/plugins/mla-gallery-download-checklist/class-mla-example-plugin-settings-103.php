 ...
 * @since 1.00
 */
class MLAExamplePluginSettings103 {
	/**
...
//error_log( __LINE__ . " MLAExamplePluginSettings103::__construct() _REQUEST = " . var_export( $_REQUEST, true ), 0 );
			...
			if ( isset( $_REQUEST[ $this->current_arguments['slug_prefix'] . '_options_save'] ) ) {
				if ( isset( $_REQUEST[ $this->current_arguments['slug_prefix'] . '_options' ] ) ) {
					if ( check_admin_referer( MLACore::MLA_ADMIN_NONCE_ACTION, MLACore::MLA_ADMIN_NONCE_NAME ) ) {
						$this->request_settings = array_map( 'sanitize_text_field', wp_unslash (  $_REQUEST[ $this->current_arguments['slug_prefix'] . '_options' ] ) );
					}
				}
			} elseif ( isset( $_REQUEST[ $this->current_arguments['slug_prefix'] . '_options_reset'] ) ) {
				$this->request_settings = $this->default_settings;
			}
//error_log( __LINE__ . ' MLAExamplePluginSettings103::__construct request_settings = ' . var_export( $this->request_settings, true ), 0 );
//error_log( __LINE__ . ' MLAExamplePluginSettings103::__construct mla_hex_dump( exports ) = ' . var_export( MLAData::mla_hex_dump( $this->request_settings['exports'] ), true ), 0 );
...
//error_log( __LINE__ . ' MLAExamplePluginSettings103::_compose_general_tab page_values = ' . var_export( $page_values, true ), 0 );
	...
	private function _save_setting_changes() {
		...
		$changed = false;
//error_log( __LINE__ . ' MLAExamplePluginSettings103::_save_setting_changes current_arguments[options] = ' . var_export( $this->current_arguments['options'], true ), 0 );
		foreach ( $this->current_arguments['options'] as $slug => $option ) {
			if ( 'checkbox' === $option['type'] ) {
		...
		$this->request_settings = array();
//error_log( __LINE__ . " MLAExamplePluginSettings103::_save_setting_changes( {$changed} ) final current_settings = " . var_export( $this->current_settings, true ), 0 );
...
//error_log( __LINE__ . ' MLAExamplePluginSettings103::_get_plugin_settings stored current_settings = ' . var_export( $this->current_settings, true ), 0 );
...
//error_log( __LINE__ . ' MLAExamplePluginSettings103::_get_plugin_settings final current_settings = ' . var_export( $this->current_settings, true ), 0 );
		return true;
	}
