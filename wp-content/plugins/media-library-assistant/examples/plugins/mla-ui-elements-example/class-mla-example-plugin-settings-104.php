...
/**
 * Class MLA Example Plugin Settings adds Settings/ General and Documentation submenu pages to an example plugin
 *
 * @package MLA Example Plugin Settings
 * @since 1.00
 */
class MLAExamplePluginSettings104 {
	/**
...
//error_log( __LINE__ . " MLAExamplePluginSettings104::__construct() _REQUEST = " . var_export( $_REQUEST, true ), 0 );
//error_log( __LINE__ . ' MLAExamplePluginSettings104::__construct mla_hex_dump( exports ) = ' . var_export( MLAData::mla_hex_dump( $_REQUEST[ $this->current_arguments['slug_prefix'] . '_options' ]['exports'] ), true ), 0 );
		...
		if ( is_admin() ) {
			...
			if ( isset( $_REQUEST[ $this->current_arguments['slug_prefix'] . '_options_save'] ) ) {
				if ( isset( $_REQUEST[ $this->current_arguments['slug_prefix'] . '_options' ] ) ) {
					if ( check_admin_referer( MLACore::MLA_ADMIN_NONCE_ACTION, MLACore::MLA_ADMIN_NONCE_NAME ) ) {
						$this->request_settings = array_map( 'wp_kses_post', wp_unslash (  $_REQUEST[ $this->current_arguments['slug_prefix'] . '_options' ] ) );
					}
				}
			} elseif ( isset( $_REQUEST[ $this->current_arguments['slug_prefix'] . '_options_reset'] ) ) {
				$this->request_settings = $this->default_settings;
			}
//error_log( __LINE__ . ' MLAExamplePluginSettings104::__construct request_settings = ' . var_export( $this->request_settings, true ), 0 );
//error_log( __LINE__ . ' MLAExamplePluginSettings104::__construct mla_hex_dump( exports ) = ' . var_export( MLAData::mla_hex_dump( $this->request_settings['exports'] ), true ), 0 );
	...
	public function admin_menu() {
		 ...
		 * There is no nonce available for varifying the tab suffix, so we have to trust the URL.
		 * The tab suffix is not used for any security validation.
		 */
		$tab = 'general';
		 if ( isset( $_REQUEST['page'] ) && is_string( $_REQUEST['page'] ) ) {
			$page = sanitize_text_field( wp_unslash( $_REQUEST['page'] ) );
		 	if ( $this->current_arguments['slug_prefix'] . '-settings-' == substr( $page, 0, strlen( $this->current_arguments['slug_prefix'] . '-settings-' ) ) ) {
				$tab = substr( $page, strlen( $this->current_arguments['slug_prefix'] . '-settings-' ) );
			}
		 }
	 ...
	 * @since 1.00
	 *
	 * @param	array 	$links array of links for the Plugin, e.g., "Activate"
	...
	public function add_submenu_page() {
		if ( !current_user_can( 'manage_options' ) ) {
			echo wp_kses_post( '<h2>' . $this->current_arguments['plugin_title'] . " - Error</h2>\n" );
		...
		if ( isset( $_REQUEST['mla_tab'] ) ) {
			$current_tab_slug = trim( sanitize_text_field( wp_unslash( $_REQUEST['mla_tab'] ) ) );
		} else {
			$current_tab_slug = 'general';
		}
		...
		// Compose plugin-specific messages
		if ( !empty( $this->current_arguments['messages'] ) ) {
			if ( false !== strpos( $this->current_arguments['messages'], 'ERROR' ) ) {
			...
			$page_values['messages'] = MLAData::mla_parse_template( $this->page_template_array['messages'], array(
				 ...
				 'messages' => $this->current_arguments['messages'],
			) );
		}
		...
		if ( ! empty( $page_content['message'] ) ) {
			...
			// Add to any plugin-specific messages
			$page_values['messages'] .= MLAData::mla_parse_template( $this->page_template_array['messages'], array(
				 'mla_messages_class' => $messages_class ,
		...
		/*
		 * The template and the values have been escaped already, in the _compose... functions.
	...
	private function _compose_general_tab() {
		...
		foreach ( $this->current_arguments['options'] as $slug => $option ) {
			if ( 'checkbox' === $option['type'] ) {
				...
				$page_values[ $slug  ] = esc_textarea( $this->get_plugin_option( $slug ) );
			} else {
				$page_values[ $slug  ] = esc_attr( $this->get_plugin_option( $slug ) );
			}
		}
//error_log( __LINE__ . ' MLAExamplePluginSettings104::_compose_general_tab page_values = ' . var_export( $page_values, true ), 0 );
		...
		$page_values = array_merge( array(
			...
			), $this->current_arguments['general_tab_values']
		);
	...
	private function _save_setting_changes() {
		...
		$changed = false;
error_log( __LINE__ . ' MLAExamplePluginSettings104::_save_setting_changes current_arguments[options] = ' . var_export( $this->current_arguments['options'], true ), 0 );
		foreach ( $this->current_arguments['options'] as $slug => $option ) {
			if ( 'checkbox' === $option['type'] ) {
		...
		$this->request_settings = array();
error_log( __LINE__ . " MLAExamplePluginSettings104::_save_setting_changes( {$changed} ) final current_settings = " . var_export( $this->current_settings, true ), 0 );
...
//error_log( __LINE__ . ' MLAExamplePluginSettings104::_get_plugin_settings stored current_settings = ' . var_export( $this->current_settings, true ), 0 );
...
//error_log( __LINE__ . ' MLAExamplePluginSettings104::_get_plugin_settings final current_settings = ' . var_export( $this->current_settings, true ), 0 );
		return true;
	}
	 ...
	 *
	 * @param string	$name Argument name
	 * @param mixed		$new_value Argument value
	 *
...
//error_log( __LINE__ . " update_plugin_argument( {$name} ) new_value()= " . var_export( $new_value, true ), 0 );
