...
* New: For PNG files, the new "MLA Diffusion Parameters Example" plugin parses the "parameters" tEXt chunk values for image files generated by a diffusion image generator.
* New: Two new filters have been added to the "Field-level substitution parameter filters (Hooks)" to support adding or modifying Attachment File Metadata values.
* New: The Media Manager Modal (popup) window "Set Featured Image" mode now supports table views based on custom field quries, etc.
* New: The "MLA Not Featured View Example" plugin has been enhanced to support the Media/Library grid view and the Media Manager Modal (popup) window.
* New: Extensive "Where-used Reporting" coverage with application examples has been added to the Settings/Media Library Assistant Documentation tab.
* New: EXIF metadata embedded in WebP image files is parsed and added to the Attachment File Metadata arrays. It can be accessed with the `exif:` prefix. XMP metadata is also supported.
* New: The ".gdz" and ".ged" file types (GEnealogical Data COMmunications, GEDCOM) have been added to MLA's optional list of Known Types.
* Fix: On the Settings/Media Library Assistant General tab, the "Import ALL Settings" button is displayed (in a disabled state) before any settings have been exported.
* Fix: File locations for the Media Manager Enhancement filters (Hooks) have been corrected in the Documentation tab and the "MLA Media Modal Hooks Example" plugin.
* Fix: New MIME Types added by themes or plugins are now added to MLA's list when first encountered. They can be edited in the Settings/Media LIbrary Assistant Uploads tab.
* Fix: In the Bulk Edit area, flat taxonomies using the checklist format with the checked on top option are now formatted correctly.
* Fix: For flat taxonomies, MLA's checklist format processing is not applied to checklists generated by other plugins, e.g., Modern Event Calendar.
* Fix: When the "+ Add New Term" feature is used to add a term to a flat taxonomy with a checklist-style metabox the new term is properly assigned to the item(s) being added/edited.
* Fix: MLA debug logging for MIME Type processing is suppressed for WordPress AJAX "heartbeat" requests.
* Fix: The Chateau Seaview Fund donation link has been updated to reflect changes at the ALS Association web site.
